...
impl Software3D {
    ...
    pub fn draw_view(
            ...
            .and_then(|m| self.find_player_subsector_id(subsectors, &m.subsector))
        {
            ...
            if self.use_edge_spans {
                // Edge-span path: emit edges during BSP walk, then process
                // scanlines and draw spans.
                let use_pvs = pvs.is_visible(player_subsector_id, player_subsector_id);
                ...
                // buffer.
            } else if self.use_bsp_occlusion {
            ...
            #[cfg(feature = "render_stats")]
            {
                ...
                if elapsed >= 1.0 {
                    ...
                    if self.use_edge_spans {
                        println!("edge-spans: {}", self.edge_state.stats);
                    }
    ...
    fn prepare_and_emit_polygon(
        ...
        let interpolator =
            match render::TriangleInterpolator::new(screen_verts, tex_coords, inv_w_slice) {
                Some(interp) => interp,
                None => return false,
            };
