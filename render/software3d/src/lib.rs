...
use level::{
    AABB, BSP3D, LevelData, Sector, SurfaceKind, SurfacePolygon, WallTexPin, WallType,
...
pub struct Software3D {
    ...
    fov: f32,
    /// Frustum FOVs (radians) from the OG projection.
    hfov: f32,
    vfov: f32,
    view_matrix: Mat4,
    camera_pos: Vec3,
    ...
    /// World-space side planes for node/leaf culling, rebuilt per frame.
    frustum: Frustum,
...
impl Software3D {
    pub fn new(width: f32, height: f32, fov: f32, debug_draw: DebugDrawOptions) -> Self {
        let mut s = Self {
            ...
            // Placeholder; rebuilt from the live view before each frame.
            frustum: Frustum::new(Vec3::ZERO, 0.0, 0.0, 1.0, 1.0),
    ...
    fn update_view_matrix(&mut self, view: &RenderView) {
        ...
        self.frustum = Frustum::new(pos, angle, pitch, self.hfov, self.vfov);
    }
    ...
    /// Hi-Z occlusion: true if the box is fully behind existing depth.
    fn cull_bbox_hiz(&self, bbox: &AABB) -> bool {
        ...
        // Project to screen AABB if all corners are in front
    ...
    pub fn draw_view_bench(
        ...
        self.view_projection = self.projection_matrix * self.view_matrix;
        self.frustum = Frustum::new(pos, angle_rad, pitch_rad, self.hfov, self.vfov);
    ...
    fn render_bsp(
        ...
        if is_subsector(node_id) {
            ...
            if self.cull_bbox_hiz(&leaf.aabb) {
