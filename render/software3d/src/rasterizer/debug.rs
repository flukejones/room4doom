...
use super::polygon::MIN_GEOMETRY_DEPTH;
use super::sampling::TextureSampler;
...
impl Software3D {
    ...
    pub(crate) fn draw_polygon_debug(
        ...
        let mut y_start = bounds.0.y.max(0.0).ceil() as u32 as usize;
        let mut y_end = bounds.1.y.min(height_f32 - 1.0).floor() as u32 as usize;
        // Sub-pixel-height polygon: ceil/floor inverted the Y range.
        // Snap to the nearest integer scanline.
        if y_start > y_end {
            let mid = ((bounds.0.y + bounds.1.y) * 0.5).round() as u32 as usize;
            if mid < self.height as usize {
                y_start = mid;
                y_end = mid;
            } else {
                return;
        ...
        for y in y_start..=y_end {
            ...
            if found < 2 {
                // Sub-pixel-height polygon (all edges nearly horizontal):
                // use AABB x-bounds and average inv_w for this scanline.
                if (bounds.1.y - bounds.0.y) < 1.0 {
                    x0 = bounds.0.x;
                    x1 = bounds.1.x;
                    let sum_iw: f32 = inv_w_slice[..vertex_count].iter().sum();
                    let avg_iw = sum_iw / vertex_count as f32;
                    inv_w_at_x0 = avg_iw;
                    inv_w_at_x1 = avg_iw;
                } else {
                    continue;
            ...
            if is_sky {
                ...
                let mut x = x_start;
                while x <= x_end {
                    self.rasterizer.depth_buffer.set_sky_depth_unchecked(x, y);
                    x += 1;
                }
                did_draw = true;
            } else {
                ...
                while x <= x_end {
                    ...
                    if !no_depth {
                        while x <= x_end {
                            ...
                            if test_inv_w > self.rasterizer.depth_buffer.peek_depth_unchecked(x, y)
                            {
                                break;
                            }
                            interp_state.step_x();
                    ...
                    while x <= x_end {
                        ...
                        if is_masked {
                            if !no_depth
                                && edge_inv_w
                                    <= self.rasterizer.depth_buffer.peek_depth_unchecked(x, y)
                            {
                                interp_state.step_x();
                                edge_inv_w += edge_inv_w_dx;
                                x += 1;
