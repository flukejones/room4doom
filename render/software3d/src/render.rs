...
impl Software3D {
    ...
    pub(super) fn draw_polygon_debug(
        ...
        let inv_w_slice = &self.inv_w_buffer[..self.inv_w_len];
        let alpha = self.debug_draw.alpha;
        let no_depth = self.debug_draw.no_depth;
        let colour_mode = &self.debug_draw.colour_mode;
        ...
        for y in y_start..=y_end {
            ...
            while x <= x_end {
                ...
                while x <= x_end {
                    ...
                    if is_masked {
                        if !no_depth && edge_inv_w <= self.depth_buffer.peek_depth_unchecked(x, y) {
                            interp_state.step_x();
                            edge_inv_w += edge_inv_w_dx;
                            x += 1;
                            break;
    ...
    pub(super) fn draw_debug_normal_lines(&mut self, buffer: &mut impl DrawBuffer) {
        let lines = std::mem::take(&mut self.debug_normal_lines);
