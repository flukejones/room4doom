...
fn test_frustum_clipping() {
    ...
    renderer
        .rasterizer
        .clip_polygon_frustum(&vertices, &tex_coords, 3);

    // Clipped polygon should have at least 3 vertices
    assert!(renderer.rasterizer.clipped_vertices_len >= 3);
    assert!(renderer.rasterizer.clipped_vertices_len <= MAX_CLIPPED_VERTICES);

    // All clipped vertices should be within frustum bounds
    for i in 0..renderer.rasterizer.clipped_vertices_len {
        let v = renderer.rasterizer.clipped_vertices_buffer[i];
        assert!(
...
fn test_frustum_clipping_completely_outside() {
    ...
    assert_eq!(renderer.rasterizer.clipped_vertices_len, 0);
}
...
fn test_clip_against_single_plane() {
    ...
    renderer.rasterizer.clipped_vertices_buffer[0] = Vec4::new(-2.0, 0.0, 0.0, 3.0); // Inside
    renderer.rasterizer.clipped_vertices_buffer[1] = Vec4::new(0.5, 0.0, 0.0, 3.0); // Inside
    renderer.rasterizer.clipped_vertices_buffer[2] = Vec4::new(-5.0, 0.5, 0.0, 3.0); // Outside left

    renderer.rasterizer.clipped_tex_coords_buffer[0] = Vec3::new(0.0, 0.0, 1.0);
    renderer.rasterizer.clipped_tex_coords_buffer[1] = Vec3::new(1.0, 0.0, 1.0);
    renderer.rasterizer.clipped_tex_coords_buffer[2] = Vec3::new(0.5, 1.0, 1.0);

    renderer.rasterizer.clipped_vertices_len = 3;

    // Clip against left plane: x >= -w
    // clip_polygon_against_plane is private, so use clip_polygon_frustum
    // with a triangle that only crosses the left plane
    let vertices = [
        Vec4::new(-2.0, 0.0, 0.0, 3.0), // Inside
        Vec4::new(0.5, 0.0, 0.0, 3.0),  // Inside
        Vec4::new(-5.0, 0.5, 0.0, 3.0), // Outside left
    ];
    let tex_coords = [
        Vec3::new(0.0, 0.0, 1.0),
        Vec3::new(1.0, 0.0, 1.0),
    ...
    assert_eq!(renderer.rasterizer.clipped_vertices_len, 4);

    // All resulting vertices should satisfy the left plane equation
