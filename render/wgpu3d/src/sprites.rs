...
impl SpriteScratch {
    ...
    /// Walk every sector's thinglist, resolve each thing's sprite frame/rotation
    /// (replicating software3d), and build a back-to-front-sorted billboard
    /// instance list. No PVS — GPU depth + a behind-camera reject handle
    /// occlusion (over-draw is acceptable). Opaque instances precede fuzz.
    pub fn collect(
        &mut self,
        view: &RenderView,
        ...
        pic_data: &PicData,
        projection: Mat4,
        voxel_mgr: Option<&VoxelManager>,
    ) {
        let player_pos = Vec3::new(view.x.into(), view.y.into(), view.viewz.into());
        let player_xy = Vec2::new(view.x.into(), view.y.into());
        let view_proj = projection * Mat4::look_at_rh(Vec3::ZERO, view_forward(view), Vec3::Z);
        let frac = view.frac;
        ...
        for sector in level_data.sectors.iter() {
            <Sector as SectorExt>::run_func_on_thinglist(sector, |thing: &MapObject| {
                ...
                if let Some(mgr) = voxel_mgr
                    && is_voxel_eligible(mgr, thing, player_xy.x, player_xy.y)
                {
                    return true;
                }
                ...
                let lerp = |prev: FixedT, curr: FixedT| {
                    prev.to_f32() + (curr.to_f32() - prev.to_f32()) * frac
                };
                ...
                let rel = Vec3::new(
                    thing.x.to_f32() - player_pos.x,
                    thing.y.to_f32() - player_pos.y,
                    thing.z.to_f32() - player_pos.z,
                );
