...
impl ScenePipeline {
    ...
    /// Draw the scene into the colour + depth views. `sky_bind` (group 3) lets
    /// sky-flagged walls sample the same sky as the background pass.
    pub fn draw(
        &self,
        ...
        rpass.draw(0..mesh.corner_count, 0..1);
    }
}
...
/// `corner_scroll`, `sector_light`) are retained for re-upload.
pub struct GpuMesh {
