...
impl Software25D {
    ...
    /// - Projects X linearly (OG `R_ProjectSprite`) so edges clip off-screen
    ...
    fn project_sprite(
        ...
        // OG R_ProjectSprite linear projection: x = centerxfrac + tx * xscale.
        // A LUT/angle mapping wraps at the FOV edge and pins the sprite to the
        // screen border instead of letting it scroll off; linear x1/x2 go
        // arbitrarily out of range and clip correctly.
        let centerx = FixedT::from(screen_width as i32 / 2);
        let x1 = centerx + tx * x_scale;
        if x1 > FixedT::from(screen_width as i32) {
            return true;
        }

        tx += FixedT::from(patch.data.len() as i32);
        let x2 = (centerx + tx * x_scale) - 1;
        if x2 < 0 {
            return true;
        }
