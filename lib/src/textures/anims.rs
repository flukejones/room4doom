    ...
    for def in ANIM_DEFS {
        ...
        if def.is_texture {
            ...
            if animation.numpics < 2 {
                panic!("init_animations: bad cycle from {} to {}", def.start_name, def.end_name);
            }
...
impl AnimationDef {
    const fn new(is_texture: bool, end_name: &'static str, start_name: &'static str, speed: usize) -> Self {
        Self {
            is_texture,
            end_name,
            start_name,
