...
impl Software3D {
    ...
    fn render_surface_polygon(
        ...
        for i in 0..VERT_COUNT {
            ...
            if let Some((p1, tex1, w1)) = projected_vertices[v1_idx] {
            ...
            // Handle edge crossing near plane regardless of vertex visibility
            if v1_view.z < -self.near_z && v2_view.z > -self.near_z {
                ...
                if clip_pos.w > 0.0 {
                    ...
                    let (u_clip, v_clip) =
                        self.calculate_tex_coords(clip_point_world, &polygon, bsp3d, pic_data);
            ...
            } else if v1_view.z > -self.near_z && v2_view.z < -self.near_z {
                let t = (-self.near_z - v1_view.z) / (v2_view.z - v1_view.z);
                let clip_point_view = v1_view + (v2_view - v1_view) * t;
                let clip_point_world = v1_world + (v2_world - v1_world) * t;
    ...
    fn calculate_polygon_depth(&mut self, polygon: &SurfacePolygon, bsp3d: &BSP3D) -> f32 {
        ...
        for &vertex_idx in &polygon.vertices {
            let (view_pos, _) = self.get_transformed_vertex(vertex_idx, bsp3d);
