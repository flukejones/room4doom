use crate::{PVS, Segment};

#[test]
fn test_pvs_creation() {
    let pvs = PVS::new(4);
    assert_eq!(pvs.subsector_count, 4);
    // 4*4 = 16 bits, (16+7)/8 = 2 bytes
    assert_eq!(pvs.visibility_data.len(), 2);
}
...
fn test_visibility_setting() {
    ...
    pvs.set_visible(1, 2, false);

    assert!(pvs.is_visible(0, 1));
    assert!(!pvs.is_visible(1, 2));
    assert!(pvs.is_visible(0, 2)); // Default is visible (bit=0)
}

#[test]
fn test_pvs_integration() {
    // Test that PVS integrates properly with subsectors and segments
    let pvs = PVS::new(2);
    ...
    assert_eq!(visible.len(), 2); // Default is visible (bit=0 means visible)
