...
mod pvs_tests {
    ...
    #[test]
    fn test_e1m2_linedef_978_981_should_be_blocked() {
        let wad_path = match find_wad_file() {
            Some(path) => path,
            None => {
                eprintln!("No WAD files found, skipping test");
        ...
        // Find subsectors containing linedef 978 and 981
        let linedef_978_subsectors = find_linedef_subsectors(&map_data, 978);
        let linedef_981_subsectors = find_linedef_subsectors(&map_data, 981);

        println!("=== Linedef 978 to 981 Blocking Test ===");
        println!("Linedef 978 is in subsectors: {:?}", linedef_978_subsectors);
        println!("Linedef 981 is in subsectors: {:?}", linedef_981_subsectors);

        // Test visibility between these subsectors - should be blocked
        let mut any_visible = false;
        for &source in &linedef_978_subsectors {
            for &target in &linedef_981_subsectors {
                let visible = map_data.bsp_3d.subsector_visible(source, target);
                let distance = calculate_subsector_distance(&map_data, source, target);

                println!(
                    "Subsector {} (linedef 978) -> Subsector {} (linedef 981): {} (distance: {:.2})",
                    source,
                    target,
                    if visible { "VISIBLE" } else { "BLOCKED" },
                    distance
        ...
        // These should be blocked by intervening geometry
        assert!(
            !any_visible,
            "Linedef 978 subsectors should NOT be visible from linedef 981 subsectors due to blocking geometry"
        );
    }

    #[test]
    fn test_e1m2_linedef_287_should_not_see_110_377() {
        let wad_path = match find_wad_file() {
            Some(path) => path,
            None => {
                eprintln!("No WAD files found, skipping test");
        ...
        // Find subsectors containing these linedefs
        let linedef_287_subsectors = find_linedef_subsectors(&map_data, 287);
        let linedef_110_subsectors = find_linedef_subsectors(&map_data, 110);
        let linedef_377_subsectors = find_linedef_subsectors(&map_data, 377);

        println!("=== Height Separation Blocking Test ===");
        println!("Linedef 287 is in subsectors: {:?}", linedef_287_subsectors);
        println!("Linedef 110 is in subsectors: {:?}", linedef_110_subsectors);
        println!("Linedef 377 is in subsectors: {:?}", linedef_377_subsectors);

        // Test that linedef 287 cannot see linedef 110
        let mut any_287_sees_110 = false;
        for &source in &linedef_287_subsectors {
            for &target in &linedef_110_subsectors {
                let visible = map_data.bsp_3d.subsector_visible(source, target);
                if visible {
                    any_287_sees_110 = true;
                    let distance = calculate_subsector_distance(&map_data, source, target);
                    println!(
                        "Linedef 287 subsector {} -> Linedef 110 subsector {}: VISIBLE (distance: {:.2})",
        ...
        // Test that linedef 287 cannot see linedef 377
        let mut any_287_sees_377 = false;
        for &source in &linedef_287_subsectors {
            for &target in &linedef_377_subsectors {
                let visible = map_data.bsp_3d.subsector_visible(source, target);
                if visible {
                    any_287_sees_377 = true;
                    let distance = calculate_subsector_distance(&map_data, source, target);
                    println!(
                        "Linedef 287 subsector {} -> Linedef 377 subsector {}: VISIBLE (distance: {:.2})",
                        source, target, distance
                    );
                }
            }
        ...
        // These should be blocked due to height separation
        assert!(
            !any_287_sees_110,
            "Linedef 287 should NOT see linedef 110 due to height separation"
        );
        assert!(
            !any_287_sees_377,
            "Linedef 287 should NOT see linedef 377 due to height separation"
        );
    }
}
