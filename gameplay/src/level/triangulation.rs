...
pub fn carve_subsector_polygon(segments: &[Segment], divlines: &[DivLine]) -> Vec<Vec2> {
    ...
    let final_polygon = add_missing_edge_vertices(&clipped_points, segments);
...
fn add_missing_edge_vertices(polygon: &[Vec2], segments: &[Segment]) -> Vec<Vec2> {
    if polygon.len() < 3 {
    ...
    // Collect all segment vertices
    let mut segment_vertices = Vec::new();
    for segment in segments {
        segment_vertices.push(*segment.v1);
        segment_vertices.push(*segment.v2);
    }
    ...
    for i in 0..polygon.len() {
        ...
        // Check each segment vertex to see if it lies on this edge
        for &seg_vertex in &segment_vertices {
            ...
            if projection >= -EPSILON && projection <= 1.0 + EPSILON {
                ...
                if distance_to_line < EPSILON {
                    // Vertex lies on this edge - add it with its projection parameter
