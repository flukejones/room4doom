...
impl MapData {
    ...
    pub fn load(&mut self, map_name: &str, pic_data: &PicData, wad: &WadData) {
        ...
        let mut vertexes = self.load_vertexes(map_name, wad, extended.as_ref());
        self.load_sectors(map_name, wad, pic_data);
        self.load_sidedefs(map_name, wad, &tex_order);
        self.load_linedefs(map_name, wad, &vertexes);
        self.prepass_fix_vertices(map_name, wad, &mut vertexes, extended.as_ref());
        ...
        self.load_segments(map_name, wad, &vertexes, extended.as_ref());
    ...
    fn load_linedefs(&mut self, map_name: &str, wad: &WadData, vertexes: &[Vec2]) {
