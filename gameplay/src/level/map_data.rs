...
impl MapData {
    ...
    pub fn vertexes_mut(&mut self) -> &mut [Vec2] {
        &mut self.vertexes
    }
    ...
    /// Build PVS data for this map
    pub fn build_pvs(&mut self) {
    ...
    /// Get PVS data if available
    pub fn pvs(&self) -> Option<&PVS> {
        self.pvs.as_ref()
    }

    /// Check if one subsector can see another using PVS
    pub fn subsector_visible(&self, from: usize, to: usize) -> bool {
        if let Some(ref pvs) = self.pvs {
            pvs.is_visible(from, to)
        } else {
            // If no PVS data, assume everything is visible
            true
        }
    }

    /// Get all visible subsectors from a given subsector
    pub fn get_visible_subsectors(&self, from: usize) -> Vec<usize> {
        if let Some(ref pvs) = self.pvs {
            pvs.get_visible_subsectors(from)
        } else {
            // If no PVS data, return all subsectors
            (0..self.subsectors.len()).collect()
        }
    }
}
