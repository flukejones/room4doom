...
pub fn p_use_special_line(_side: i32, line: MapPtr<LineDef>, thing: &mut MapObject) -> bool {
    ...
    if generalized::is_generalized(line.special) {
        return generalized::handle_generalized_use(line, thing, level);
    }
    ...
    match line.special {
        ...
        11 => {
            change_switch_texture(line, false, &level.switch_list, &mut level.button_list, &level.snd_command, &mut level.level_data.bsp_3d);
            level.do_exit_level();
        }
        51 => {
            change_switch_texture(line, false, &level.switch_list, &mut level.button_list, &level.snd_command, &mut level.level_data.bsp_3d);
        ...
        42 => {
            debug!("line-switch: vld_close door!");
            if ev_do_door(line.clone(), DoorKind::Close, level) {
                change_switch_texture(line, true, &level.switch_list, &mut level.button_list, &level.snd_command, &mut level.level_data.bsp_3d);
            }
        }
        61 => {
            debug!("line-switch: vld_open door!");
        ...
        133 | 99 => {
            if let Some(player) = thing.player_mut() {
                if player.status.cards[Card::Bluecard as usize] || player.status.cards[Card::Blueskull as usize] {
                    change_switch_texture(line.clone(), line.special == 99, &level.switch_list, &mut level.button_list, &level.snd_command, &mut level.level_data.bsp_3d);
        ...
        134 | 135 => {
            if let Some(player) = thing.player_mut() {
                if player.status.cards[Card::Redcard as usize] || player.status.cards[Card::Redskull as usize] {
                    change_switch_texture(line.clone(), line.special == 134, &level.switch_list, &mut level.button_list, &level.snd_command, &mut level.level_data.bsp_3d);
                    ev_do_door(line, DoorKind::BlazeOpen, level);
                } else {
        ...
        136 | 137 => {
            if let Some(player) = thing.player_mut() {
                if player.status.cards[Card::Yellowcard as usize] || player.status.cards[Card::Yellowskull as usize] {
                    change_switch_texture(line.clone(), line.special == 136, &level.switch_list, &mut level.button_list, &level.snd_command, &mut level.level_data.bsp_3d);
                    ev_do_door(line, DoorKind::BlazeOpen, level);
                } else {
