...
use log::{debug, warn};
use math::{Angle, FixedT, get_prndindex, get_rndindex, set_prndindex, set_rndindex};
...
fn index_to_state(idx: u16) -> Result<&'static crate::info::StateData, SaveError> {
    let max = StateNum::Count as u16;
    if idx >= max {
        return Err(SaveError::InvalidStateNum(idx));
    }
    Ok(&STATES[idx as usize])
}
...
impl SaveWriter {
    ...
    fn write_fixed(&mut self, v: FixedT) {
        self.write_i64((v.raw() as i64) << (32 - math::FRACBITS));
    }
...
impl<'a> SaveReader<'a> {
    ...
    fn read_fixed(&mut self) -> Result<FixedT, SaveError> {
        let raw = self.read_i64()?;
        Ok(FixedT((raw >> (32 - math::FRACBITS)) as math::Inner))
    }
...
/// Write the fixed-size save file header (magic, version, map info, RNG state).
...
/// Read and validate the save file header. Returns error on magic/version
/// mismatch.
...
/// Restore sector, linedef, and sidedef state from save data.
...
fn load_world(r: &mut SaveReader, level_data: &mut LevelData) -> Result<(), SaveError> {
    ...
    for i in 0..count {
        sectors[i].floorheight = SectorHeight::from_fixed(r.read_i32()?);
        sectors[i].ceilingheight = SectorHeight::from_fixed(r.read_i32()?);
    ...
    for i in 0..count {
        ...
        sidedefs[i].rowoffset = r.read_fixed()?;
        let top = r.read_i32()?;
...
/// Return the sector index from its `num` field.
fn sector_index(s: &MapPtr<Sector>) -> u32 {
    s.num as u32
}

/// Serialize a map object's full state, including its player linkage index (-1
/// if none).
fn save_mobj_with_player_index(w: &mut SaveWriter, m: &MapObject, player_idx: i8) {
    w.write_f32(m.x.to_f32());
    w.write_f32(m.y.to_f32());
    w.write_f32(m.z.to_f32());
    w.write_f32(m.angle.rad());
    w.write_u16(m.sprite as u16);
    w.write_u32(m.frame);
    w.write_f32(m.floorz.to_f32());
    w.write_f32(m.ceilingz.to_f32());
    w.write_f32(m.radius.to_f32());
    w.write_f32(m.height.to_f32());
    w.write_f32(m.momx.to_f32());
    w.write_f32(m.momy.to_f32());
    w.write_f32(m.momz.to_f32());
...
/// lights) from save data. Dispatches on per-thinker type tag.
fn load_thinkers(
    r: &mut SaveReader,
...
/// Deserialize a single map object, insert it into the thinker list,
/// set its position in the blockmap, and re-link to its player if applicable.
fn load_mobj(
    r: &mut SaveReader,
    level: &mut LevelState,
    players: &mut [Player],
...
/// Deserialize a vertical door thinker and attach it to its sector.
fn load_vdoor(r: &mut SaveReader, level: &mut LevelState) -> Result<(), SaveError> {
    let sector_num = r.read_u32()?;
    let kind = r.read_u8()?;
    let topheight = SectorHeight::from_fixed(r.read_i32()?);
    let speed = SectorHeight::from_fixed(r.read_i32()?);
    let direction = r.read_i32()?;
    let topwait = r.read_i32()?;
    let topcountdown = r.read_i32()?;

    let sector = resolve_sector(sector_num, &mut level.level_data)?;
...
/// Deserialize a floor-move thinker and attach it to its sector.
fn load_floor(r: &mut SaveReader, level: &mut LevelState) -> Result<(), SaveError> {
    let sector_num = r.read_u32()?;
    let kind = r.read_u8()?;
    let speed = SectorHeight::from_fixed(r.read_i32()?);
...
/// Deserialize a ceiling-move thinker and attach it to its sector.
fn load_ceiling(r: &mut SaveReader, level: &mut LevelState) -> Result<(), SaveError> {
    let sector_num = r.read_u32()?;
    let kind = r.read_u8()?;
    let bottomheight = SectorHeight::from_fixed(r.read_i32()?);
    let topheight = SectorHeight::from_fixed(r.read_i32()?);
    let speed = SectorHeight::from_fixed(r.read_i32()?);
    let crush = r.read_bool()?;
    let direction = r.read_i32()?;
...
fn load_platform(r: &mut SaveReader, level: &mut LevelState) -> Result<(), SaveError> {
    ...
    let high = SectorHeight::from_fixed(r.read_i32()?);
    let wait = r.read_i32()?;
...
/// Deserialize a light flash thinker.
fn load_light_flash(r: &mut SaveReader, level: &mut LevelState) -> Result<(), SaveError> {
    let sector_num = r.read_u32()?;
    let count = r.read_i32()?;
...
/// Deserialize a strobe flash thinker.
fn load_strobe(r: &mut SaveReader, level: &mut LevelState) -> Result<(), SaveError> {
    let sector_num = r.read_u32()?;
    let count = r.read_i32()?;
    let min_light = r.read_u32()? as usize;
    let max_light = r.read_u32()? as usize;
...
fn load_fire_flicker(r: &mut SaveReader, level: &mut LevelState) -> Result<(), SaveError> {
    let sector_num = r.read_u32()?;
    let count = r.read_i32()?;
    let max_light = r.read_u32()? as usize;
    let min_light = r.read_u32()? as usize;
...
fn load_glow(r: &mut SaveReader, level: &mut LevelState) -> Result<(), SaveError> {
    let sector_num = r.read_u32()?;
    let min_light = r.read_u32()? as usize;
    let max_light = r.read_u32()? as usize;
    let direction = r.read_i32()?;
...
/// Serialize all player state: status, inventory, weapons, psprites, frags.
fn save_players(w: &mut SaveWriter, players: &[Player], players_in_game: &[bool; MAXPLAYERS]) {
    for i in 0..MAXPLAYERS {
        ...
        w.write_f32(p.viewz.to_f32());
        w.write_f32(p.viewheight.to_f32());
        w.write_f32(p.deltaviewheight.to_f32());
        w.write_f32(p.bob.to_f32());
...
/// Restore all player state from save data (status, inventory, weapons,
/// psprites).
fn load_players(
    r: &mut SaveReader,
    players: &mut [Player],
    ...
    for i in 0..MAXPLAYERS {
        ...
        p.viewz = FixedT::from_f32(r.read_f32()?);
        p.viewheight = FixedT::from_f32(r.read_f32()?);
        p.deltaviewheight = FixedT::from_f32(r.read_f32()?);
        p.bob = FixedT::from_f32(r.read_f32()?);
...
/// Serialize active switch/button timers.
fn save_buttons(w: &mut SaveWriter, level: &LevelState) {
...
/// Restore active switch/button timers from save data.
fn load_buttons(r: &mut SaveReader, level: &mut LevelState) -> Result<(), SaveError> {
...
/// Serialize the thing respawn queue (deathmatch).
fn save_respawn_queue(w: &mut SaveWriter, level: &LevelState) {
...
/// Restore the thing respawn queue from save data.
fn load_respawn_queue(r: &mut SaveReader, level: &mut LevelState) -> Result<(), SaveError> {
    let count = r.read_u32()? as usize;
...
fn save_level_stats(w: &mut SaveWriter, level: &LevelState) {
    w.write_i32(level.total_level_kills);
    w.write_i32(level.total_level_items);
    w.write_i32(level.total_level_secrets);
}

fn load_level_stats(r: &mut SaveReader, level: &mut LevelState) -> Result<(), SaveError> {
...
pub fn save_game_to_bytes(
    ...
    save_world(&mut w, &level.level_data);
    ...
    level.thinkers.for_each(|t| match t.data() {
        ...
        ThinkerData::VerticalDoor(d) => {
            ...
            w.write_i32(d.topheight.to_fixed_raw());
            w.write_i32(d.speed.to_fixed_raw());
        ...
        ThinkerData::CeilingMove(c) => {
            ...
            w.write_i32(c.bottomheight.to_fixed_raw());
            w.write_i32(c.topheight.to_fixed_raw());
            w.write_i32(c.speed.to_fixed_raw());
        ...
        ThinkerData::Platform(p) => {
            ...
            w.write_i32(p.speed.to_fixed_raw());
            w.write_i32(p.low.to_fixed_raw());
            w.write_i32(p.high.to_fixed_raw());
            w.write_i32(p.wait);
            w.write_i32(p.count);
...
pub fn load_game_from_bytes(
    ...
    load_world(&mut r, &mut level.level_data)?;
