...
/// Lost Soul charge speed in map units/tic (OG `SKULLSPEED = 20*FRACUNIT`).
/// The charge uses this constant, not the skull's walk `info.speed`.
...
pub(crate) fn a_skullattack(actor: &mut MapObject) {
    if let Some(target) = actor.target {
        ...
        let bam = actor.angle.to_bam();
        let speed = FixedT::from_int(SKULLSPEED);
...
pub(crate) fn a_tracer(actor: &mut MapObject) {
    ...
    if let Some(dest) = actor.tracer {
        ...
        let speed = FixedT(actor.info.speed);
...
#[cfg(test)]
mod enemy_tests {
    use super::{a_facetarget, a_posattack, a_skullattack, a_troopattack};
    use crate::MapObjKind;
    use crate::test_support::{TestLevel, rng_guard};
    use crate::thing::MapObjFlag;
    use math::get_prndindex;

    /// Lost Soul charge: `a_skullattack` sets Skullfly and launches the skull
    /// toward its target with nonzero horizontal momentum.
    #[test]
    fn skullattack_launches_charge() {
        let _g = rng_guard();
        let mut level = TestLevel::load("E1M1");
        // Target 64 units east of the skull.
        let target = level.spawn_ptr(1120, -3616, MapObjKind::MT_TROOP);
        let skull = level.spawn(1056, -3616, MapObjKind::MT_SKULL);
        skull.target = Some(unsafe { (*target).thinker });

        a_skullattack(skull);

        assert!(
            skull.flags.contains(MapObjFlag::Skullfly),
            "skull must enter Skullfly"
        );
        // Target due east: full charge speed goes into momx. OG uses
        // SKULLSPEED (20 map units/tic), NOT info.speed (8). Allow finecosine
        // table rounding (cos≈0.99998 -> ~19.999 units).
        assert!(
            (19..=20).contains(&skull.momx.to_i32()),
            "skull charges at ~SKULLSPEED (20), got momx={:?}",
            skull.momx
        );
        assert!(skull.momy.to_i32().abs() <= 1, "no lateral drift due east");
    }
