...
impl OplPlayerState {
    ...
    /// Parse and load MIDI data, replacing any existing track. Returns Err on
    /// malformed input.
    ...
    /// Reset channel state, silence active notes, and begin playback from the
    /// start.
    ...
    pub fn generate_samples(&mut self, buffer: &mut [i16]) {
        ...
        for sample in buffer.iter_mut() {
            ...
            while self.samples_per_tick > 0.0 && self.sample_counter >= self.samples_per_tick {
    ...
    /// Load and parse the GENMIDI lump from the WAD.
    ///
    /// Returns (main_instruments[128], percussion_instruments[47]).
    /// Falls back to silent default instruments if the lump is missing or
    /// malformed.
    ...
    /// Parse a standard MIDI file into a flat event list sorted by absolute
    /// tick time.
    ///
    /// - Reads MThd header for format, track count, and ticks-per-quarter
    /// - Iterates MTrk chunks, decoding delta times and running status
    /// - Extracts note on/off, control change, program change, pitch bend, and
    ///   tempo meta events
    ...
    fn parse_variable_length(&self, data: &[u8], mut pos: usize) -> Result<(u32, usize), String> {
        ...
        loop {
            ...
            if bytes_read >= 4 {
                return Err("Variable-length value exceeds 4 bytes".to_string());
            }
            byte = data[pos];
            pos += 1;
            bytes_read += 1;
    ...
    /// Steal the lowest-priority active voice to free an OPL channel.
    ///
    /// Priority: second voices of double-voice instruments are stolen first,
    /// then voices on higher-numbered MIDI channels. Sustained notes are never
    /// stolen.
    ...
    /// Program OPL registers for an instrument voice on the given channel.
    ///
    /// Writes modulator and carrier operator registers (tremolo, attack,
    /// sustain, waveform, scale/level) and the channel feedback/connection
    /// register. For FM mode, the modulator level is set from the
    /// instrument data; for AM mode, it is set to max attenuation
    /// (carrier-only volume control).
    fn setup_instrument(
        &mut self,
        opl_channel: usize,
    ...
    /// Calculate OPL frequency register value and octave for a MIDI note.
    ///
    /// Accounts for the instrument's base note offset (or fixed note),
    /// pitch bend, and fine tuning (for second voices). Looks up the
    /// result in `FREQUENCY_CURVE`.
    ...
    /// Recalculate and write OPL frequency registers for an active voice (used
    /// after pitch bend changes).
    ...
    /// Allocate an OPL channel and activate a single instrument voice.
    ///
    /// Programs the instrument, sets volume, calculates frequency with pitch
    /// bend, and writes the key-on registers. Returns `(opl_channel,
    /// freq_value, reg_volume)` on success, or `None` if no channel could
    /// be allocated.
    fn key_on_voice(
        &mut self,
        channel: u8,
    ...
    ///
    /// - Resolves the instrument from program number (or percussion map for
    ///   channel 9)
    /// - Allocates an OPL channel (stealing if necessary), programs the
    ///   instrument, sets volume, and triggers key-on
    /// - For double-voice instruments, allocates a second OPL channel for voice
    ///   1
    ...
    ///
    /// If the sustain pedal is active, marks the note as sustained instead of
    /// releasing it. Otherwise, clears the key-on bit and frees the OPL
    /// channel.
    ...
    /// Release all notes marked as sustained on the given MIDI channel (sustain
    /// pedal off).
    ...
    /// Process all MIDI events whose absolute time has been reached.
    ///
    /// Dispatches note on/off, control changes (volume, expression, sustain,
    /// all-sound-off, all-notes-off), program changes, pitch bend, and tempo
    /// meta events. Handles track looping when enabled.
