...
/// Map an envelope rate value to a (table_index, right_shift) pair for rate
/// lookup.
/// - val < 52: standard range, index = val & 3, shift = 12 - (val >> 2)
/// - val 52..59: high range, index = val - 48, shift = 0
/// - val >= 60: maximum rate, index = 12, shift = 0
...
impl Operator {
    /// Recompute attack rate increment from register 0x60 upper nibble and KSR.
    ...
    /// Recompute decay rate increment from register 0x60 lower nibble and KSR.
    ...
    /// Recompute release rate increment from register 0x80 lower nibble and
    /// KSR. Also updates the sustain-phase zero flag based on the SUSTAIN
    /// bit in reg 0x20.
    ...
    /// Recalculate KSR from channel key code and refresh all envelope rates if
    /// changed.
    ...
    /// Recompute total attenuation level from register 0x40 (TL + KSL scaling).
    ...
    /// Recompute wave phase increment and vibrato depth from channel
    /// frequency/block data. Uses `wrapping_mul` to avoid overflow on high
    /// frequencies.
    fn update_frequency(&mut self) {
        let freq = self.chan_data & 0x3FF;
        let block = (self.chan_data >> 10) & 0x07;
    ...
    /// Handle write to register 0x20-0x35 (tremolo, vibrato, sustain, KSR,
    ...
    /// Handle write to register 0x40-0x55 (total level and key scale level).
    ...
    /// Handle write to register 0x60-0x75 (attack rate in upper nibble, decay
    /// rate in lower).
    ...
    /// Handle write to register 0x80-0x95 (sustain level in upper nibble,
    /// release rate in lower).
    ...
    /// Handle write to register 0xE0-0xF5 (waveform select).
    ...
    /// Advance the envelope state machine and return the current envelope
    /// volume.
    ///
    /// Transitions through Attack -> Decay -> Sustain -> Release -> Off,
    /// applying the appropriate rate increment at each stage.
    ...
    /// Look up the waveform sample at `index` and scale by the envelope volume
    /// `vol`.
    ...
    /// Generate one output sample with phase modulation applied.
    /// Returns 0 and advances the wave index if the envelope is silent.
    ...
    /// Pre-compute per-sample values: current attenuation (with tremolo) and
    /// wave increment (with vibrato).
...
impl Channel {
    /// Update channel frequency/keycode data and propagate to both operators.
    ...
    /// Recalculate KSL base and key code from current frequency data, then
    /// update both operators.
    fn update_frequency(&mut self, reg08: u8, attack_rates: &[u32; 76], linear_rates: &[u32; 76]) {
        let mut data = self.chan_data & 0xFFFF;
        let ksl_base = ksl_tbl()[((data >> 6) & 0x7F) as usize] as u32;
    ...
    /// Handle write to register 0xA0-0xA8 (frequency number low byte).
    fn write_a0(
        &mut self,
        val: u8,
        reg08: u8,
    ...
    /// Handle write to register 0xB0-0xB8 (key-on, block number, frequency high
    /// bits).
    fn write_b0(
        &mut self,
        val: u8,
    ...
    /// Handle write to register 0xC0-0xC8 (feedback depth, synthesis mode, OPL3
    /// panning).
    ...
    /// Generate `samples` worth of audio for this channel and mix into
    /// `output`.
    ///
    /// - FM mode: modulator feeds carrier via phase modulation
    /// - AM mode: modulator and carrier outputs are summed
    /// - Skips generation entirely if all active operators are silent
...
impl Chip {
    /// Call `setup(rate)` before generating audio.
    pub fn new() -> Self {
        let mut chip = Self {
    ...
    /// Configure chip for the given output sample rate. Computes frequency
    /// multipliers, LFO increments, and calibrates attack/linear rate tables.
    ...
    /// Write to an OPL register. Registers < 0x100 target OPL2/low bank;
    /// >= 0x100 target OPL3 high bank.
    ...
    /// Advance the LFO (vibrato + tremolo) counters and return how many samples
    /// can be generated before the next LFO tick boundary.
    ...
    /// Handle write to register 0xBD (rhythm mode, vibrato/tremolo depth,
