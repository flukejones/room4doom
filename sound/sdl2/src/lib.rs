...
impl<'a> SoundServer<SfxName, sdl2::Error> for Snd<'a> {
    fn init(&mut self) -> InitResult<SfxName, sdl2::Error> {
        Ok(self.tx.clone())
    }
    ...
    fn start_music(&mut self, data: Vec<u8>, looping: bool) {
        ...
        if self.use_opl2 {
            if let Some(ref mut opl) = self.opl_player {
                if opl.load_music(&data).is_ok() && opl.play(looping).is_ok() {
                    opl.set_volume(self.mus_vol);
                    debug!("Playing music with OPL2");
                    return;
                }
            }
        }
        // Leak the data so SDL_mixer can hold a 'static reference.
        // Music changes are infrequent so this is acceptable.
        let leaked: &'static [u8] = Vec::leak(data);
        if let Ok(music) = Music::from_static_bytes(leaked) {
...
impl<'a> SoundServerTic<SfxName, sdl2::Error> for Snd<'a> {}
...
mod tests {
    ...
    #[test]
    fn mus_to_midi_conversion() {
        let wad = WadData::new(&doom1_wad_path());
        let lump = wad.get_lump("D_E1M1").expect("D_E1M1 lump missing");
        let midi = read_mus_to_midi(&lump.data).expect("MUS to MIDI conversion failed");
        assert!(!midi.is_empty());
        // MIDI starts with "MThd" header
        assert_eq!(&midi[..4], b"MThd", "MIDI header missing");
    }
}
