...
use geom_kernel::{
    EditorMap, GROWTH_HEADROOM, LineDef, LineFlags, Sector, SideDef, Thing, ThingFlags, Vertex,
};
...
impl fmt::Display for DwdError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Io(e) => write!(f, "dwd io error: {e}"),
            Self::BadHeader {
                line,
                found,
            } => {
...
impl From<io::Error> for DwdError {
...
fn parse_side(s: &mut Cursor<'_, DwdError>, intern: &mut Interner) -> Result<SideDef, DwdError> {
    let y_offset = s.int()?;
    s.lit("(")?;
    let x_offset = s.int()?;
    s.lit(":")?;
...
/// Read and parse a .dwd file (DoomEd import; the editor writes native maps).
pub fn load_dwd(path: &Path) -> Result<EditorMap, DwdError> {
...
mod tests {
    ...
    fn fixture_counts() {
        ...
        assert_eq!(map.lines.iter().filter(|l| l.back.is_some()).count(), 173);
    ...
    fn fixture_first_line_record_fields() {
        ...
        let sec = map.sectors[line.front.sector.expect("dwd side has a sector") as usize];
    ...
    fn fixture_two_sided_record_fields() {
        ...
        let front_sec = map.sectors[line.front.sector.expect("front sector") as usize];
        let back_sec = map.sectors[back.sector.expect("back sector") as usize];
        assert_eq!(
    ...
    fn thing_snap_applies_on_load() {
        ...
        assert_eq!((map.things[0].x, map.things[0].y), (992, -16));
    }
