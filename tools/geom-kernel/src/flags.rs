//! Typed linedef + thing flag bits mirroring `level::LineDefFlags` (vanilla plus BOOM's `PassUse`); `flags`/`options` round-trip verbatim as raw `i32` (byte-compatible with old plain-integer RON/undo), and unnamed bits are retained, not dropped.

use bitflags::bitflags;
use serde::{Deserialize, Deserializer, Serialize, Serializer};

bitflags! {
    /// Linedef flags (classic Doom `ML_*`); the editor interprets only [`LineFlags::TWO_SIDED`], the rest round-trip through import/export.
...
bitflags! {
    /// Thing option flags (vanilla `MTF_*`): difficulty presence, deaf/ambush, multiplayer-only; drives the editor's skill filter and thing panel.
    #[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
