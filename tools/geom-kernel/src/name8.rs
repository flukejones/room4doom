//! 8-byte, NUL-padded texture/flat/patch names with DoomEd `char[9]` semantics; all-NUL means "no texture", the .dwd format uses `"-"` for empty, and eq/hash operate on the zero-padded bytes matching DoomEd's `bcmp` comparison exactly.
...
pub(crate) const DWD_EMPTY_NAME: &str = "-";

/// An 8-byte, NUL-padded name, canonicalized to uppercase on construction (Doom names are case-insensitive, so eq/hash need no per-call folding); serializes as the plain uppercase string (empty → `""`) for readable RON/JSON.
...
impl Name8 {
    ...
    /// Build from a string of 1..=8 printable, non-whitespace ASCII bytes, uppercased; empty input → [`Name8::EMPTY`].
