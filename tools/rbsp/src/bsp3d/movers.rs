...
///
/// Specials are normalised to generalized form by the input prep (see
...
pub fn classify_special(special: u32) -> Option<MoverKind> {
...
fn build_tag_linedef_index(input: &Bsp3dInput) -> HashMap<i16, Vec<usize>> {
    let mut map: HashMap<i16, Vec<usize>> = HashMap::new();
    for (li, ld) in input.linedefs.iter().enumerate() {
        if ld.tag != 0 {
...
fn classify_sector_mover(
    ...
    {
        for &li in indices {
            if let Some(kind) = classify_special(input.linedefs[li].special) {
    ...
    for &li in &sector.lines {
        let ld = &input.linedefs[li as usize];
        let back = ld.sides[1];
        if back != NO_REF
            && input.sidedefs[back as usize].sector as usize == sector_id
            && let Some(kind) = classify_special(ld.special)
        {
            result = Some(match result {
                Some(prev) => prev.combine(kind),
                None => kind,
...
impl Bsp3dBuilder {
    // ------------------------------------------------------------------
    // Entry point (called from Bsp3dBuilder::build)
    // ------------------------------------------------------------------
    ...
    pub(super) fn mover_vertex_pass(&mut self, input: &Bsp3dInput) {
        ...
        let tag_linedefs = build_tag_linedef_index(input);

        for (i, sector) in input.sectors.iter().enumerate() {
            if let Some(kind) = classify_sector_mover(input, i, &tag_linedefs) {
                mover_sectors.insert(i, kind);
            }
            if (sector.ceil_h - sector.floor_h).abs() <= HEIGHT_EPSILON {
                zh_sectors.insert(i);
            }
        }

        // Build sector→segment indices for O(1) lookup per sector.
        let mut sector_segs: Vec<Vec<usize>> = vec![Vec::new(); input.sectors.len()];
        for (si, seg) in input.segs.iter().enumerate() {
            let front_id = seg.frontsector as usize;
            if front_id < input.sectors.len() {
                sector_segs[front_id].push(si);
            }
            if seg.backsector != NO_REF {
                let back_id = seg.backsector as usize;
                if back_id < input.sectors.len() && back_id != front_id {
        ...
        for seg in &input.segs {
            ...
            let front_id = seg.frontsector as usize;
            let back_id = seg.backsector as usize;
            let front = &input.sectors[front_id];
            let back = &input.sectors[back_id];
            let sidedef = &input.sidedefs
                [input.linedefs[seg.linedef as usize].sides[seg.side as usize] as usize];

            if sidedef.has_bottom && (front.floor_h - back.floor_h).abs() <= HEIGHT_EPSILON {
                ...
                for sv in [input.verts[seg.v1 as usize], input.verts[seg.v2 as usize]] {
                    push_dedup_bound(
                        &mut zh_lower_bounds,
                        &mut zh_lower_seen,
                        sv,
            ...
            if sidedef.has_top && (front.ceil_h - back.ceil_h).abs() <= HEIGHT_EPSILON {
        ...
        for seg in &input.segs {
            ...
            let front = &input.sectors[front_id];
            let back = &input.sectors[back_id];
            if (front.floor_h - back.floor_h).abs() <= HEIGHT_EPSILON {
                for sv in [input.verts[seg.v1 as usize], input.verts[seg.v2 as usize]] {
                    push_dedup_bound(
                        &mut zh_lower_bounds,
                        &mut zh_lower_seen,
            ...
            if (front.ceil_h - back.ceil_h).abs() <= HEIGHT_EPSILON {
                for sv in [input.verts[seg.v1 as usize], input.verts[seg.v2 as usize]] {
                    push_dedup_bound(
                        &mut zh_upper_bounds,
                        &mut zh_upper_seen,
        ...
        for seg in &input.segs {
            ...
            let front = &input.sectors[front_id];
            let back = &input.sectors[back_id];
            if (front.floor_h - back.ceil_h).abs() <= HEIGHT_EPSILON {
            ...
            if (back.floor_h - front.ceil_h).abs() <= HEIGHT_EPSILON {
                floor_ceil_sectors.insert(front_id);
                floor_ceil_sectors.insert(back_id);
                for sv in [input.verts[seg.v1 as usize], input.verts[seg.v2 as usize]] {
                    push_dedup_bound(
        ...
        for &sector_id in &all_relevant {
            let boundary_pts = self.collect_boundary_points(input, sector_id, &sector_segs);
            let floor_h = input.sectors[sector_id].floor_h;
            let ceil_h = input.sectors[sector_id].ceil_h;
        ...
        for &sector_id in &zh_sectors {
            let height = input.sectors[sector_id].floor_h;
        ...
        let ceil_map_sectors: BTreeSet<usize> = mover_sectors
            ...
            .chain(zh_sectors.iter().copied())
            .collect();
        self.populate_vertex_map_from_polys(input, &floor_map_sectors, &mut lower_vertex_map, true);
        self.populate_vertex_map_from_polys(input, &ceil_map_sectors, &mut upper_vertex_map, false);
        ...
        self.connect_non_zh_wall_vertices(input, &lower_vertex_map, &upper_vertex_map);
    ...
    /// A leaf's own + shared polygon indices, cloned for iteration while
    /// mutating `self` (the pre-split code iterated `polygon_indices`, which
    /// contained both).
    fn leaf_all_polys(&self, ss_id: usize) -> Vec<usize> {
        let leaf = &self.leaves[ss_id];
        leaf.polys.iter().chain(&leaf.shared).copied().collect()
    }

    /// Collect all seg endpoint positions touching `sector_id`, deduplicated
    /// within `DEDUP_EPSILON`.
    fn collect_boundary_points(
        ...
        for &si in &sector_segs[sector_id] {
            let seg = &input.segs[si];
            add(input.verts[seg.v1 as usize], &mut seen, &mut pts);
            add(input.verts[seg.v2 as usize], &mut seen, &mut pts);
            // Also collect vertices from adjacent back-sector subsectors.
            let front_id = seg.frontsector as usize;
            if front_id == sector_id && seg.backsector != NO_REF {
                let back_num = seg.backsector as usize;
                for &ss_id in &self.sector_subsectors[back_num] {
                    ...
                    for gs in &input.segs[start..end] {
                        add(input.verts[gs.v1 as usize], &mut seen, &mut pts);
                        add(input.verts[gs.v2 as usize], &mut seen, &mut pts);
                    }
                }
            }
        }
    ...
    fn separate_zh_surface(&mut self, sector_id: usize, is_floor: bool) {
        ...
        for ss_id in 0..self.leaves.len() {
            for gi in self.leaf_all_polys(ss_id) {
        ...
        for si in 0..self.sector_subsectors[sector_id].len() {
            ...
            let polys = if is_floor {
                self.leaves[ss_id].floor_polygons.clone()
            } else {
                self.leaves[ss_id].ceiling_polygons.clone()
            };
            for gi in polys {
    ...
    fn separate_boundary_vertices(
        ...
        for &(pos, sector_a, sector_b) in bounds {
            let height = if is_floor {
                input.sectors[sector_a].floor_h
            } else {
                input.sectors[sector_a].ceil_h
            };
    ...
    fn separate_cross_height_vertices(
        ...
        for &(pos, floor_sector, ceil_sector) in bounds {
            let height = input.sectors[floor_sector].floor_h;
            let qp = QuantizedVec2::from_vec2(pos, QUANT_PRECISION);
    ...
    fn populate_vertex_map_from_polys(
        ...
        for &sector_id in sector_ids {
            let height = if is_floor {
                input.sectors[sector_id].floor_h
            } else {
                input.sectors[sector_id].ceil_h
            };
            for i in 0..self.sector_subsectors[sector_id].len() {
                let ss_id = self.sector_subsectors[sector_id][i];
                let leaf = &self.leaves[ss_id];
    ...
    fn connect_non_zh_wall_vertices(
        ...
        for ss_id in 0..self.leaves.len() {
            for gi in self.leaf_all_polys(ss_id) {
                ...
                let (wall_type, linedef_id) = match &poly.kind {
                    BuildKind::Wall {
                        wall_type,
                        linedef,
                        ..
                    } => (*wall_type, *linedef as usize),
                    BuildKind::Flat => continue,
                };
                ...
                let ld = &input.linedefs[linedef_id];
                if ld.sides[0] == NO_REF || ld.sides[1] == NO_REF {
                    continue;
                }
                let ld_front = input.sidedefs[ld.sides[0] as usize].sector as usize;
                let ld_back = input.sidedefs[ld.sides[1] as usize].sector as usize;
                    ...
                    match wall_type {
                        WallType::Lower => {
                            self.try_link_wall_vertex(
                                ...
                                &qp,
                                v.z,
                                input.sectors[wall_front].floor_h,
                                wall_front,
                                lower_vertex_map,
                            );
                            self.try_link_wall_vertex(
                                ...
                                &qp,
                                v.z,
                                input.sectors[wall_back].floor_h,
                                wall_back,
                                lower_vertex_map,
                            );
                        }
                        WallType::Upper => {
                            self.try_link_wall_vertex(
                                ...
                                &qp,
                                v.z,
                                input.sectors[wall_front].ceil_h,
                                wall_front,
                                upper_vertex_map,
                            );
                            self.try_link_wall_vertex(
                                ...
                                &qp,
                                v.z,
                                input.sectors[wall_back].ceil_h,
                                wall_back,
                                upper_vertex_map,
                            );
                        }
    ...
    fn set_mover_flags(
        ...
        for &sector_id in &floor_movers {
            for i in 0..self.sector_subsectors[sector_id].len() {
                let ss_id = self.sector_subsectors[sector_id][i];
                for gi in self.leaves[ss_id].floor_polygons.clone() {
        ...
        for &sector_id in &ceil_movers {
            for i in 0..self.sector_subsectors[sector_id].len() {
                let ss_id = self.sector_subsectors[sector_id][i];
                for gi in self.leaves[ss_id].ceiling_polygons.clone() {
                    self.polygons[gi].moves = true;
                }
            }
        }
        ...
        for ss_id in 0..self.leaves.len() {
            for gi in self.leaf_all_polys(ss_id) {
                let poly = &self.polygons[gi];
                let linedef_id = match &poly.kind {
                    BuildKind::Wall {
                        linedef,
