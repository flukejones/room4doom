//! Editor asset model: palette, colormaps, textures, patches, animations.
//! No `pic-data` dep; palette/colormaps from raw `PLAYPAL`/`COLORMAP` lumps.
...
/// Monotonic per-kind edit counters; caches use these to detect staleness.
...
pub(crate) struct EditorAssets {
    ...
    /// Patches flattened to palette indices per resolved texture (lazy, like `init_wall_pics`).
    /// Evicted on edit; cleared on map-wad/group change.
...
impl EditorAssets {
    /// Load assets from `wad_paths` (IWAD first). Each path is opened separately
    /// so `TEXTURE<n>` lumps get provenance-tagged groups; the merged `wad` view
    /// supplies palette / colormaps / flats / animations.
    pub fn load(wad_paths: &[PathBuf], wad: &WadData, project: Option<&Project>) -> Self {
        ...
        for path in wad_paths {
            let name = path
                ...
                .unwrap_or("?")
                .to_string();
            let w = WadData::new(path);
            texture_groups.extend(import_wad_texture_groups(&name, &w));
        }
        ...
        let texture_mode = project.map_or(TextureMode::default(), |p| p.settings.texture_mode);
        let mut assets = Self {
            palette,
            colormaps,
            texture_groups,
    ...
    pub fn set_map_wad(&mut self, map_wad: &str) {
        ...
        self.active_map_wad = map_wad.to_string();
        self.rebuild_texture_index();
    }
    ...
    /// Rebuild name → `(group, def)` in reverse load order (later WAD/lump wins).
    /// Custom: all groups merge. Vanilla: only `active_map_wad` groups. Clears `composed`.
    ...
    pub fn texture_def(&self, name: &Name8) -> Option<&TextureDef> {
        ...
        self.texture_groups.get(g).and_then(|grp| grp.defs.get(t))
    }

    /// Map-referenced names missing from the WAD set. Derived live.
    /// Unresolved wall texture/flat → `Texture`. Resolved texture with absent patch lump → `Patch`.
    /// (Patches of unresolved textures are unknowable.) Deduped, name-sorted.
    ...
    pub(super) fn replace_textures(&mut self, set: Vec<TextureDef>) -> Vec<TextureDef> {
        ...
        let g = &mut self.texture_groups[self.active_group];
        g.edited = true;
        let prev = std::mem::replace(&mut g.defs, set);
        self.rebuild_texture_index();
        prev
    }
...
mod tests {
    ...
    fn def_for<'a>(a: &'a EditorAssets, name: &str) -> Option<&'a TextureDef> {
        ...
        a.texture_groups.get(g).and_then(|grp| grp.defs.get(t))
    }
