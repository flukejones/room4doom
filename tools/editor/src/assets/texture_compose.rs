//! Doom picture codec and texture composer — pure palette-index data, no UI.
//! `gfx` builds Slint images on top; no Slint dep here avoids a module cycle.
...
/// Texel covered by a missing patch lump. Distinct from [`TRANSPARENT_INDEX`] so
/// the atlas packer shows magenta instead of see-through. Out of palette range.
...
/// Decode Doom picture format: `{width, height, offsets i16}`, u32 column offsets,
/// per-column posts `{topdelta u8 (0xFF=end), length u8, pad, pixels, pad}`.
...
/// Encode row-major palette indices as a Doom picture lump. Heights over
/// [`MAX_PATCH_HEIGHT`] are capped; tall-patch not implemented.
...
/// Compose a texture to palette indices, column-major (`data[x*h+y]`).
/// Cached by [`super::EditorAssets::ensure_composed`].
///
/// If any patch lump is missing (dims unknowable), all uncovered texels become
/// [`MISSING_PATCH_INDEX`] (magenta) instead of [`TRANSPARENT_INDEX`]. Present
/// patches draw normally; genuine transparency only survives if no patch is missing.
...
mod tests {
    ...
    fn missing_patch_fills_uncovered_texels_with_sentinel() {
        ...
        assert!(
            pic.data.iter().any(|&i| i == MISSING_PATCH_INDEX),
            "a missing patch yields sentinel texels"
        );
        assert!(
            !pic.data.iter().any(|&i| i == TRANSPARENT_INDEX),
