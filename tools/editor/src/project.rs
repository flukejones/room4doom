...
use crate::generated::{
    ...
    ResourceEntry, ResourcesController, WallPreview,
};
...
pub(crate) fn refresh_recent(ui: &EditorWindow, shared: &Rc<RefCell<SharedState>>) {
    let entries: Vec<slint::SharedString> = shared
        ...
        .collect();
    ui.global::<RecentController>()
        .set_entries(slint::ModelRc::new(VecModel::from(entries)));
}

/// Recompute missing-resources and push to panel. Call after atlas refresh so
/// the texture index reflects this map's WAD.
pub(crate) fn refresh_resources(ui: &EditorWindow, shared: &Rc<RefCell<SharedState>>) {
    ...
    ctl.set_entries(slint::ModelRc::new(VecModel::from(entries)));
}
...
/// Open the WAD. Requested/sole map loads directly; else pops map picker.
/// No project → creates a draft (materialised on first Save).
pub(crate) fn open_wad(
    ...
    match requested {
        ...
        None => {
            ...
            maps_ctl.set_maps(slint::ModelRc::new(VecModel::from(model)));
...
pub(crate) fn load_wad_file(shared: &Rc<RefCell<SharedState>>, path: &Path) -> WadLoad {
    ...
    if !state.add_pwad(path.to_path_buf()) {
        return WadLoad::Skipped;
    }
    WadLoad::Pwad
}
