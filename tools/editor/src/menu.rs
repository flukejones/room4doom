...
use crate::jobs;
use crate::png_export::{PNG_SCALE_PRESETS, export_png};
use crate::project::{
...
pub(crate) fn set_callbacks_menu(ui: &EditorWindow, shared: &Rc<RefCell<SharedState>>) {
    ...
    ui.on_menu_open_recent(move |path| {
        ...
        open_project(&ui, &s, std::path::Path::new(path.as_str()));
        refresh_all(&ui, &s);
    });
...
fn wire_png(ui: &EditorWindow, shared: &Rc<RefCell<SharedState>>) {
    ...
    ui.on_menu_export_png(move |scale| {
        ...
        let path = {
            ...
            rfd::FileDialog::new()
                .set_file_name(format!("{}.png", state.app.map_name))
                .add_filter("PNG", &["png"])
                .save_file()
        };
        let Some(path) = path else { return };
        match export_png(&s.borrow(), scale, &path) {
            Ok(()) => log::info!("wrote {}", path.display()),
            Err(e) => log::error!("png export: {e}"),
        }
    });
}
