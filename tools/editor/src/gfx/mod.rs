//! Lazily memoised asset thumbnails over [`EditorAssets`]; `WallPic`: column-major, `FlatPic`: row-major 64×64, `u16::MAX` = transparent.
...
pub use wall::{WallBand, render_flat_square, render_wall};
...
impl GfxCache {
    ...
    fn sync(&mut self, assets: &EditorAssets) {
        ...
        if now.textures != self.built_gen.textures {
            self.tex_thumbs.iter_mut().for_each(|t| *t = None);
        }
        if now.patches != self.built_gen.patches {
