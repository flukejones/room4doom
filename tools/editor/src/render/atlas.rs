//! GPU atlas textures + lookup maps for the wgpu canvas.
//!
//! Three atlases packed once per map/asset change:
//! - **Flat atlas** — one 64×64 RGBA tile per distinct flat; palette indices baked to RGBA.
//! - **Wall atlas** — shelf-packed RGBA wall textures; transparent texels (`u16::MAX`) → alpha 0.
//! - **Sprite atlas** — one RGBA rect per thing-kind icon; maps to [`SpriteSlot`] normalised UVs.
...
use crate::assets::{EditorAssets, MISSING_PATCH_INDEX, MISSING_PATCH_RGBA, TRANSPARENT_INDEX};
...
/// Flat texel side; all vanilla flats are 64×64.
const FLAT_SIDE: u32 = 64;
...
/// Atlas origin lookup maps. `*_tile` vecs rebuilt per sector edit; rest rebuilt when used-name set changes.
#[derive(Default)]
pub struct AtlasMaps {
    /// Sector → floor flat tile origin (atlas texels); `None` = unknown flat.
    pub sector_tile: Vec<Option<[f32; 2]>>,
    /// Sector → ceil flat tile origin (atlas texels); `None` = unknown flat.
    pub sector_ceil_tile: Vec<Option<[f32; 2]>>,
...
/// Recompute per-sector tile origins from `flat_name_tile`. No RGBA packing or GPU upload.
/// Run when sector flats change or for per-tic animation ticks.
pub fn remap_sector_tiles(map: &EditorMap, maps: &mut AtlasMaps) {
    maps.sector_tile = map
        .sectors
        .iter()
        .map(|s| maps.flat_name_tile.get(&s.floor_flat).copied())
        .collect();
    maps.sector_ceil_tile = map
        .sectors
        .iter()
        .map(|s| maps.flat_name_tile.get(&s.ceil_flat).copied())
        .collect();
}

/// Hash of the atlas inputs: used flat/wall names, thing kinds, asset generation.
/// Unchanged key → atlas RGBA is identical; skip pack+upload, only [`remap_sector_tiles`] needed.
pub fn content_key(assets: &EditorAssets, map: &EditorMap, sprites: &ThingSpriteCache) -> u64 {
    let mut h = FNV_OFFSET;
    let g = assets.generation();
    for v in [g.textures, g.patches, g.palette, g.animations] {
        h = fnv_fold(h, v);
    }
    ...
    flats.sort_unstable_by_key(|n| n.as_str().to_owned());
    flats.dedup();
    for n in &flats {
        h = fold_name(h, n, 0xff);
    }

    let mut walls = collect_wall_names(assets, map);
    walls.sort_unstable_by_key(|n| n.as_str().to_owned());
    for n in &walls {
...
fn pack_wall_atlas(
    ...
    for name in &names {
        ...
        if w > WALL_ATLAS_MAX || h > WALL_ATLAS_MAX {
            log::warn!(
                "wall texture {:?} ({}×{}) exceeds atlas max; skipped",
                name.as_str(),
                w,
                h
            );
...
mod tests {
    ...
    fn e1m1_flat_atlas_packs_distinct_flats() {
        ...
        assert_eq!(maps.sector_tile.len(), map.sectors.len());
        for tile in maps.sector_tile.iter().flatten() {
            assert!(
                tile[0] >= 0.0 && (tile[0] as u32) < data.flat_atlas_w,
                "floor tile origin inside atlas"
            );
        }
        assert_eq!(maps.sector_ceil_tile.len(), map.sectors.len());
        for tile in maps.sector_ceil_tile.iter().flatten() {
            assert!(
                tile[0] >= 0.0 && (tile[0] as u32) < data.flat_atlas_w,
                "ceil tile origin inside atlas"
            );
        }
    }
    ...
    fn content_key_tracks_used_names_not_geometry() {
        ...
        let key0 = content_key(&assets, &map, &sprites);
        map.vertices[0].x += 64.0; // a pure position move
        assert_eq!(
            content_key(&assets, &map, &sprites),
            key0,
        ...
        let existing = map
            ...
            .iter()
            .map(|s| s.floor_flat)
            .find(|f| *f != map.sectors[0].floor_flat)
            .expect("E1M1 has two distinct floor flats");
        map.sectors[0].floor_flat = existing;
        assert_eq!(
            content_key(&assets, &map, &sprites),
            key0,
        ...
        map.sectors[0].floor_flat = Name8::new("ZZNEWFLT").expect("name");
        assert_ne!(
            content_key(&assets, &map, &sprites),
            key0,
