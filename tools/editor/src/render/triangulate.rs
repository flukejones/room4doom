//! Per-sector polygon triangulation for filled map views. Sector outlines come from [`editor_core::sector_loops`] (outers CCW, holes CW). Each sector is triangulated by a plane sweep that handles holes natively: the outer loop and its hole loops share one vertex arena, the sweep splits the whole thing into y-monotone pieces (de Berg, *Computational Geometry* §3.2) — a hole's top is a split vertex, its bottom a merge, so the sweep's diagonals stitch holes to the outer — and each monotone piece is triangulated in linear time (§3.3). No hole bridging, no per-degeneracy special-cases. Distinct from the game's BSP3D polygoniser (which fans already-convex leaves).

use std::cmp::Ordering;
use std::collections::{HashMap, HashSet};
use std::f32::consts::TAU;

use editor_core::{EditorMap, SectorKey, SectorLoop, VertKey, sector_loops_all, sector_loops_for};
...
/// Triangulated sector fills, keyed by sector. Each entry is that sector's CCW corner triples.
#[derive(Debug, Default, Clone, PartialEq)]
pub struct SectorTris {
    pub sector: HashMap<SectorKey, Vec<[TriVert; 3]>>,
}

impl SectorTris {
    pub fn tris(&self, sector: SectorKey) -> &[[TriVert; 3]] {
        self.sector.get(&sector).map_or(&[], Vec::as_slice)
    }
}
...
pub fn build_sector_tris(map: &EditorMap) -> SectorTris {
    let mut out = SectorTris::default();
    for (key, loops) in sector_loops_all(map) {
        let mut tris = Vec::new();
        triangulate_sector(map, &loops, &mut tris);
        out.sector.insert(key, tris);
    }
    out
}

/// Re-triangulate the dirty sectors in place with ONE edge scan; removed sectors drop from the cache.
pub fn retriangulate_sectors(map: &EditorMap, sectors: &[SectorKey], cache: &mut SectorTris) {
    let mut live: Vec<SectorKey> = sectors
        .iter()
        .copied()
        .filter(|&s| {
            if map.sectors.contains(s) {
                true
            } else {
                cache.sector.remove(&s);
                false
            }
        })
        .collect();
    ...
    let mut by_sector = sector_loops_for(map, &live);
    for s in live {
        let loops = by_sector.remove(&s).unwrap_or_default();
        let mut tris = Vec::new();
        triangulate_sector(map, &loops, &mut tris);
        cache.sector.insert(s, tris);
    }
}
...
fn corner(map: &EditorMap, v: VertKey) -> TriVert {
...
/// Drop degenerate vertices: consecutive duplicates and antenna spikes (prev==next); both break the sweep's planar graph (two nodes at one point) and contribute no area.
...
/// Classify rings by nesting parity (even depth = outer, odd = hole), group holes under their immediate parent outer, orient (outer CCW, hole CW) for the sweep's interior-on-the-left rule.
...
/// Flattened outer+holes as one vertex set with boundary linkage; diagonals kept separate and merged into a planar graph at face-extraction time — never spliced in place.
struct Arena {
    verts: Vec<TriVert>,
    prev: Vec<usize>,
    next: Vec<usize>,
...
/// Partition arena into y-monotone pieces (de Berg §3.2); classifies on original topology, collects then applies diagonals. Multi-span status locates each vertex unambiguously — no nearest-edge guessing.
fn partition(arena: &Arena) -> Vec<(usize, usize)> {
    ...
    for &i in &order {
        ...
        match kind[i] {
            ...
            Kind::Merge => {
                // i ends the right wall of its left span and the left wall of its right span; fuse those two specific spans into one.
...
/// Extract y-monotone faces from boundary + diagonals as a directed planar graph. Face traversal: at each vertex, leave on the most clockwise half-edge from arrival — robust for multi-diagonal vertices. Single CW face (outer hull) is dropped.
...
mod tests {
    use super::*;
    use editor_core::{
        ArenaKey as _, DenseLineDef, DenseMap, DenseSideDef, LineFlags, Vertex as MapVertex,
    };
    ...
    fn side(sector: Option<u32>) -> DenseSideDef {
        DenseSideDef {
            x_offset: 0,
            y_offset: 0,
    ...
    fn wall(v1: u32, v2: u32, sector: u32) -> DenseLineDef {
        DenseLineDef {
            v1,
            v2,
    ...
    fn portal(v1: u32, v2: u32, front: u32, back: u32) -> DenseLineDef {
        DenseLineDef {
            v1,
            v2,
            flags: LineFlags::empty(),
    ...
    fn sector(st: &SectorTris, map: &EditorMap, s: usize) -> Vec<[TriVert; 3]> {
        let key = map.sectors.keys().nth(s).expect("sector exists");
        st.tris(key).to_vec()
    }
    ...
    /// Real Sunder MAP19 sector 9129: a hole touches the outer ring at a single shared vertex. Two coincident graph nodes confuse the angular face walk; the general fix (welding) regressed other sectors, so this degenerate boundary-touching-hole case is a documented limitation (≈9/11699 sectors on the hardest PWAD). Floors still render; the fill is over-covered here.
    #[test]
    ...
    fn convex_quad() {
        ...
        assert!((fill_area(&sector(&st, &map, 0)) - 16.0).abs() < 1e-3);
        assert!(all_ccw(&sector(&st, &map, 0)));
    }
    ...
    fn concave_l_shape() {
        ...
        let st = build_sector_tris(&map);
        assert!(
            (fill_area(&sector(&st, &map, 0)) - 3.0).abs() < 1e-3,
            "L area, got {}",
            fill_area(&sector(&st, &map, 0))
        );
        assert!(all_ccw(&sector(&st, &map, 0)));
    }
    ...
    fn split_vertex_w_shape() {
        ...
        assert!(
            (fill_area(&sector(&st, &map, 0)) - 12.0).abs() < 1e-3,
            "W area, got {}",
            fill_area(&sector(&st, &map, 0))
        );
        assert!(all_ccw(&sector(&st, &map, 0)));
    ...
    fn sector_with_hole() {
        ...
        let st = build_sector_tris(&map);
        assert!(
            (fill_area(&sector(&st, &map, 0)) - 32.0).abs() < 1e-3,
            "outer excludes the hole, got {}",
            fill_area(&sector(&st, &map, 0))
        );
        assert!((fill_area(&sector(&st, &map, 1)) - 4.0).abs() < 1e-3);
        assert!(all_ccw(&sector(&st, &map, 0)));
    }
    ...
    fn sector_with_two_holes() {
        ...
        let st = build_sector_tris(&map);
        assert!(
            (fill_area(&sector(&st, &map, 0)) - 92.0).abs() < 1e-3,
    ...
    fn e1m1_sectors_fill_and_wind_ccw() {
        ...
        let st = build_sector_tris(&map);
        assert_eq!(st.sector.len(), map.sectors.len());
        let all: Vec<[TriVert; 3]> = st.sector.values().flatten().copied().collect();
        let bad: Vec<_> = all
            .iter()
        ...
        assert!(all_ccw(&all), "every triangle front-facing");
        let filled = st.sector.values().filter(|t| !t.is_empty()).count();
        assert!(
    ...
    /// Nudging one vertex and re-triangulating only its sectors matches a full rebuild.
    #[test]
    fn retriangulate_dirty_sector_matches_full_build() {
        let wad = wad::WadData::new(&test_utils::doom1_wad_path());
        let mut map = editor_core::import_wad_map(&wad, "E1M1").expect("E1M1 imports");
        let mut st = build_sector_tris(&map);

        let vk = map.vertices.keys().next().expect("has vertices");
        map.vertices[vk].x += 4.0;
        let dirty: Vec<SectorKey> = map
            .lines
            .values()
            .filter(|l| l.v1 == vk || l.v2 == vk)
            .flat_map(|l| l.sides())
            .filter_map(|s| s.sector)
            .collect();
        assert!(!dirty.is_empty(), "vertex borders sectors");
        retriangulate_sectors(&map, &dirty, &mut st);

        let full = build_sector_tris(&map);
        assert_eq!(st, full, "incremental cache matches full rebuild");
    }
