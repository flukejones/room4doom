// Procedural editing-plane grid. A full-screen triangle; each pixel is unprojected through `inv_view_proj` onto the z=`grid_z` plane, then the snap grid and the 64u alignment grid are drawn from the world XY with a constant device-pixel line width (fwidth AA). Replaces the CPU-built line instances: no geometry, no per-frame rebuild, immune to grazing-ray coordinate blowup.
...
struct Grid {
    ...
    plane: vec4<f32>,
    // x = snap spacing, y = tile spacing (64), z = min device-px before a grid fades out (zoom-out cutoff), w = line half-width in device px.
    params: vec4<f32>,
    grid_rgba: vec4<f32>,
    tile_rgba: vec4<f32>,
    // x = graze-fade start |dir.z|, y = graze-fade full |dir.z|, z = far-fade start dist, w = far-fade end dist (ground units from eye).
    fade: vec4<f32>,
...
// Coverage of one grid: 1 on a line, 0 between. `spacing` in world units, `wpp` = world units per device pixel (from screen-space derivatives).
...
fn fs_grid(in: VsOut) -> FsOut {
    ...
    // Per-grid zoom-out cutoff: fade a grid as its on-screen spacing shrinks below `min_px` device px (mirrors the old `spacing*zoom < MIN` skip).
    ...
    // Horizon fade: grazing rays (|dir.z| small) and far hits fade to nothing, so the near-edge-on plane never moirés. Distance is measured on the ground from the camera eye's XY — in top-down ortho every visible cell is near the eye (no fade); only a tilted view reaches far enough to fade.
    ...
    // Real plane depth so the grid depth-tests against the 3D surface. When on-top, the pipeline's depth_compare=Always ignores this.
