// World-space colour fills + instanced screen-space lines and markers (constant
// device-pixel width, so thickness never scales with zoom).
...
// One instance per line: two endpoints, a px half-width, colour. The base quad
// is expanded in pixel space so thickness never scales with zoom.
@vertex
fn vs_line(
