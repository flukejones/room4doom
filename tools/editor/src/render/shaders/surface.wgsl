// 3D sector surfaces: floors/ceilings sample the flat atlas, walls the wall atlas, shaded by per-sector brightness. None mode draws only a selected sector's floor/ceil tint; colour mode flat-shades the sector colour with per-wall angle contrast; texture mode samples. A selected sector's floor + ceil are tinted.
...
// Shared-const values below are substituted from the Rust consts at shader build (wgpu.rs surface_wgsl); the defaults here only keep the file self-contained for wgsl-analyzer. Missing walls signal via negative rect.x.
...
// Atlas texel for wall UV `uv` within `rect` (x,y origin; z,w size). U always wraps; V wraps when `wrap_v` (solid wall), else V outside the texture height returns alpha 0 (a masked midtexture's see-through part).
...
fn fs_surface3d(in: SurfaceOut) -> @location(0) vec4<f32> {
    ...
    // Texture mode only: in Colour mode a missing texture/flat falls through to the sector's flat colour rather than drowning the view in magenta.
    ...
    // A masked midtexture keeps its transparency in every fill mode (its alpha is its shape), unlike a solid wall, which renders flat colour in Colour mode.
