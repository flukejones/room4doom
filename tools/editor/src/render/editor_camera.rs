...
pub struct EditorCamera {
    ...
    goal: Camera,
    viewport: [f32; 2],
    /// Height of the editing/grid plane that `screen_to_world` hits, set from the last contextual pick (a clicked floor/edge). 0 = the floor grid.
    grid_z: f32,
...
impl EditorCamera {
    ...
    /// World point → screen pixels. Behind camera → clamped off-screen.
