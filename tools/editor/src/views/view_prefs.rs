...
/// Combo index for `pos`; must mirror the toolbar-position ComboBox row order.
fn toolbar_position_index(pos: prefs::ToolbarPositionPref) -> i32 {
    match pos {
...
pub(crate) fn init(ui: &EditorWindow, shared: &Rc<RefCell<SharedState>>) {
    ...
    ctl.set_light_themes(model(theme_labels(ThemeVariant::Light)));
    ctl.set_dark_themes(model(theme_labels(ThemeVariant::Dark)));
    ...
    ui.global::<PrefsController>()
        .set_sector_gradients(model(gradients));
...
pub(crate) fn push_toolbar_position(ui: &EditorWindow, shared: &Rc<RefCell<SharedState>>) {
    ui.set_toolbar_position(toolbar_position_index(
        shared.borrow().prefs.toolbar_position,
    ));
}
...
fn populate(ui: &EditorWindow, shared: &Rc<RefCell<SharedState>>) {
    ...
    ctl.set_toolbar_position_index(toolbar_position_index(prefs.toolbar_position));
