//! Texture editor glue: composite textures, patches, animations.
//! No project: seeds an in-memory draft from the IWAD; Save creates a project on disk.
//! Project open: edits texture set 1 directly.
...
use crate::generated::{AnimRow, EditorWindow, TexEditController, TexPatchRow};
...
/// Texture editor tab index (see editor.slint).
const TEXTURES_TAB: i32 = 2;
...
fn texedit_key(
    ...
    if selected >= 0 {
        ...
        };
        {
            ...
            let Some(p) = def.patches.get_mut(selected as usize) else {
                return;
            };
            p.origin_x += dx;
            p.origin_y += dy;
...
fn paste_patches(ui: &EditorWindow, shared: &Rc<RefCell<SharedState>>) {
    ...
    let new_selected = {
        let state = &mut *shared.borrow_mut();
        if state.texedit.clipboard.is_empty() {
            return;
        }
        record(state);
        let clips = mem::take(&mut state.texedit.clipboard);
        let Some(def) = texture_mut(state, current) else {
            state.texedit.clipboard = clips;
            return;
        };
        ...
        let last = def.patches.len() as i32 - 1;
        state.texedit.clipboard = clips;
        last
    };
...
fn sync_list(ui: &EditorWindow, shared: &Rc<RefCell<SharedState>>) {
    ...
    ui.global::<TexEditController>()
        .set_textures(slint::ModelRc::new(VecModel::from(names)));
...
pub(crate) fn open_set(ui: &EditorWindow, shared: &Rc<RefCell<SharedState>>, index: usize) {
    ...
    ui.set_active_tab(TEXTURES_TAB);
    sync_list(ui, shared);
...
fn sync_anims(ui: &EditorWindow, shared: &Rc<RefCell<SharedState>>) {
    ...
    ui.global::<TexEditController>()
        .set_anims(slint::ModelRc::new(VecModel::from(rows)));
}
...
fn select(ui: &EditorWindow, shared: &Rc<RefCell<SharedState>>, index: i32) {
    ...
    ctl.set_patches(slint::ModelRc::new(VecModel::from(rows)));
