...
use slint::{Brush, Color, ComponentHandle as _, Model as _, ModelRc, VecModel};
use wad::WadData;
...
use crate::generated::{EditorWindow, LumpRow, ProjectBrowserController, TreeRow};
...
/// Central tab index of the Map tab (see editor.slint).
const MAP_TAB: i32 = 1;
...
fn apply_name_filter(ui: &EditorWindow, text: &str) {
    ...
    for i in 0..rows.row_count() {
        ...
        let matches = conversions::lump_matches(row.name.as_str(), &filter);
...
impl ProjectBrowserController<'_> {
    ...
    fn reset_browser_view(&self) {
        self.set_tree_rows(ModelRc::new(VecModel::from(Vec::<TreeRow>::new())));
        self.set_lump_rows(ModelRc::new(VecModel::from(Vec::<LumpRow>::new())));
...
fn select_tree_row(ui: &EditorWindow, browser: &Rc<RefCell<BrowserState>>, row: usize) {
    ...
    match leaf {
        Some(LeafRef::Wad(wad_idx)) => {
            let filter = ctl.get_name_filter().to_lowercase();
...
fn show_content(
    ...
    if kind.is_image() {
        ...
        let assets = assets.get_or_insert_with(|| {
            let mut a = EditorAssets::load(std::slice::from_ref(path), wad, None);
            a.set_map_wad(name);
            a
        });
