//! Texture/flat browser glue: "…" buttons open overlay; picks land in panel field. Thumbnails from `GfxCache`, built lazily.
...
use crate::assets::{EditorAssets, FLAT_SIDE};
...
pub(crate) fn init(ui: &EditorWindow, shared: &Rc<RefCell<SharedState>>) {
    ...
    ui.global::<LinedefInspectorController>()
        .on_populate_texture_browser(move |slot| {
            if let Some(ui) = weak.upgrade() {
                open_browser(&ui, &s, TexBrowseTarget::LineSlot(slot.into()));
            }
        });
    ...
    ui.global::<SectorInspectorController>()
        .on_populate_flat_browser(move |slot| {
            if let Some(ui) = weak.upgrade() {
                open_browser(&ui, &s, TexBrowseTarget::SectorSlot(slot.into()));
            }
        });
    ...
    ui.global::<TexEditController>()
        .on_populate_patch_browser(move || {
            if let Some(ui) = weak.upgrade() {
                open_browser(&ui, &s, TexBrowseTarget::Patch);
            }
        });
...
/// Populate the browser for `target` and show it; no-op without an IWAD.
...
fn rebuild_entries(ui: &EditorWindow, shared: &Rc<RefCell<SharedState>>) {
    ...
    if target == TexBrowseTarget::Patch {
        ...
        for name in &names {
            ...
            entries.push(patch_entry(gfx, assets, wad, name));
        }
    ...
    let active = current_slot_value(ui, target).map_or(-1, |v| {
        entry_index(entries.iter().map(|e| &e.name), v.trim())
    });
    browser.set_entries(model(entries));
    browser.set_active_index(active);
}

/// Browser entry for a named patch; zero size and blank thumb when lookup fails.
fn patch_entry(gfx: &mut GfxCache, assets: &EditorAssets, wad: &WadData, name: &str) -> GfxEntry {
...
/// Row index of `name` (ASCII case-insensitive) among `names`, or -1 when absent.
pub(crate) fn entry_index<N: AsRef<str>>(names: impl IntoIterator<Item = N>, name: &str) -> i32 {
    names
        .into_iter()
        .position(|n| n.as_ref().eq_ignore_ascii_case(name))
        .map_or(-1, |i| i as i32)
}
...
/// Case-insensitive substring filter; `filter` must be pre-folded to uppercase once per rebuild, not per row.
...
pub(crate) fn flat_entries(
    ...
    for (num, flat) in assets.iwad_flats().iter().enumerate() {
        ...
        entries.push(GfxEntry {
            ...
            tex_w: FLAT_SIDE as i32,
            tex_h: FLAT_SIDE as i32,
        });
    }
    entries
}
