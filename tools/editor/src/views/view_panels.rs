//! Inspector panel glue: selection → controller properties, apply callbacks → mutations; invalid field values fall back to the existing value.

use std::cell::RefCell;
use std::rc::Rc;

use editor_core::{
    ArenaKey, LineDef, LineFlags as LineBits, Name8, Sector, SectorKey, SideDef, Thing,
    ThingFlags as ThingBits,
};
...
fn push_specials(
    ...
    (model(labels), model(values))
}

/// Rebuild special-combo models from the open project; clear when none. Parallel label + value lists so the pick callback returns the value directly.
...
fn push_thing_types(ui: &EditorWindow, shared: &Rc<RefCell<SharedState>>) {
    let types = shared.borrow_mut().thing_types();
    ...
    ctl.set_types(model(names));
    ctl.set_type_num(model(kinds));
}
...
/// Order-independent fingerprint of a selected-key set (XOR with bias).
fn sel_key<K: ArenaKey>(keys: &[K]) -> SelKey {
    let fold = |k: &K| k.slot() ^ k.generation().rotate_left(16);
    let xor = keys.iter().fold(0u32, |a, k| a ^ fold(k).wrapping_add(1));
    (
        keys.len(),
        keys.first().map(fold).unwrap_or(u32::MAX),
        keys.last().map(fold).unwrap_or(u32::MAX),
        xor,
    )
}
...
pub(crate) fn sync(ui: &EditorWindow, shared: &Rc<RefCell<SharedState>>) {
    ...
    let lines: Vec<LineDef> = map
        .map(|m| {
            app.selected_lines()
                .into_iter()
                .filter_map(|k| m.lines.get(k).copied())
                .collect()
        })
        .unwrap_or_default();
    if lines.is_empty() {
    ...
    let things: Vec<Thing> = map
        .map(|m| {
            app.selected_things()
                .into_iter()
                .filter_map(|k| m.things.get(k).copied())
                .collect()
        })
        .unwrap_or_default();
    if !things.is_empty() {
    ...
    let sector = app
        .current_sector
        .and_then(|k| Some((k, *map?.sectors.get(k)?)));
    match sector {
        Some((key, sector)) => sync_sector(ui, key, sector),
...
/// Number as text, or blank when `None` (mixed selection).
fn opt_num(v: Option<i32>) -> slint::SharedString {
    v.map_or_else(|| "".into(), |v| slint::format!("{v}"))
}
...
fn sync_lines(ui: &EditorWindow, lines: &[LineDef]) {
    ...
    ctl.set_front(LineSide {
        ...
        xoff: opt_num(fold_common(lines, |l| l.front.x_offset)),
        yoff: opt_num(fold_common(lines, |l| l.front.y_offset)),
    });
    ...
    ctl.set_back(LineSide {
        ...
        xoff: opt_num(fold_common(lines, |l| l.back.map(|b| b.x_offset)).flatten()),
        yoff: opt_num(fold_common(lines, |l| l.back.map(|b| b.y_offset)).flatten()),
    });
...
fn sync_things(ui: &EditorWindow, things: &[Thing]) {
    ...
    ctl.set_angle(opt_num(fold_common(things, |t| t.angle)));
    let common_opt =
...
fn sync_sector(ui: &EditorWindow, key: SectorKey, sector: Sector) {
    let ctl = ui.global::<SectorInspectorController>();
    ctl.set_active(true);
    ctl.set_sector_index(key.slot() as i32);
    ...
    ui.global::<SectorInspectorController>().on_apply(move || {
        ...
        let damage = {
            ...
            let Some(old) = state
                ...
                .map
                .as_ref()
                .and_then(|m| m.sectors.get(index))
                .copied()
            else {
                return;
            };
