...
use editor_core::{EditorMap, SectorKey};
use math::p_random;
...
fn sector_neighbor_min(map: &EditorMap) -> HashMap<SectorKey, i32> {
    let mut neighbor_min: HashMap<SectorKey, i32> = map
        .sectors
        .iter()
        .map(|(k, s)| (k, s.light_level))
        .collect();
    for line in map.lines.values() {
        let front = line.front.sector;
        let back = line.back.as_ref().and_then(|b| b.sector);
...
pub(crate) fn build(map: &EditorMap) -> Vec<SectorLight> {
    ...
    for (sector, s) in map.sectors.iter() {
...
mod tests {
    use super::*;
    use editor_core::{DenseLineDef, DenseMap, DenseSideDef, LineFlags, Name8, Vertex};

    fn side(sector: u32) -> DenseSideDef {
        DenseSideDef {
            x_offset: 0,
            y_offset: 0,
            top_tex: Name8::EMPTY,
    ...
    fn two_sector_map(special: i32) -> EditorMap {
        let mut lines: Vec<DenseLineDef> = [(0u32, 1u32), (1, 2), (2, 3)]
            .into_iter()
    ...
    fn strobe_zeroes_min_when_equal_to_max() {
        ...
        let dark = map.sectors.keys().nth(1).expect("two sectors");
        map.sectors[dark].light_level = 192;
