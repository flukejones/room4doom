//! macOS window vibrancy via `NSVisualEffectView` inserted behind the window.
//! Requires `renderer-femtovg-wgpu` with a transparent swapchain.
//! winit asserts `contentView` IS its `WinitView` — replacing it panics on focus loss — so the effect
//! view is inserted as the backmost subview of the frame view (content view's superview).
