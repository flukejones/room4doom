...
pub(super) fn vert_pair(a: u32, b: u32) -> (u32, u32) {
    (a.min(b), a.max(b))
}
...
impl LevelEditorState {
    ...
    pub fn pick_3d(&self, pos: [f32; 2]) -> Option<PickHit> {
        ...
        let kind = if v.surface == SURFACE_FLOOR || v.surface == SURFACE_CEIL {
            PickKind::Sector(v.sector)
        } else {
            PickKind::Linedef(v.source)
        };
        Some(PickHit {
            kind,
            world: hit.world,
    ...
    pub fn pick_3d_select(&self, pos: [f32; 2], mode: SelectMode) -> Option<PickHit> {
        ...
        let mut vertex: Option<(f32, f32, u32, f32)> = None; // (dist², t, vert, gz)
        let mut edge: Option<(f32, f32, u32, f32)> = None; // (dist², t, line, gz)
        let mut face: Option<(f32, usize)> = None; // (t, first-vert index)
        let mut thing: Option<(f32, u32, f32)> = None; // (t, thing id, gz)
            ...
            for leaf in leaves.iter() {
                match *leaf {
                    Leaf::Tri {
                        ...
                        if matches!(mode, SelectMode::Vertex | SelectMode::All) {
                            for c in &tri {
                                ...
                                if d <= vr2 && vertex.is_none_or(|b| d < b.0) {
                                    vertex = Some((d, t, c.vert, c.pos[2]));
                                }
                            }
                        }
                        ...
                        if matches!(mode, SelectMode::Line | SelectMode::All) {
                            for (m, n) in [(0, 1), (1, 2), (2, 0)] {
                                ...
                                let Some(&li) = edge_lines.get(&vert_pair(a.vert, b.vert)) else {
                                    continue;
                                };
                                let (d, t) = seg_ray_dist_sq(a.pos, b.pos, origin, dir);
                                if d <= r2 && edge.is_none_or(|e| d < e.0) {
                                    edge = Some((d, t, li, a.pos[2]));
                                }
                            }
                        }
                    }
                    Leaf::Thing {
                        ...
                        let Some(t) = map.things.get(id as usize) else {
                            continue;
                        };
        ...
        // Vert/edge within PICK_RADIUS of the nearest solid surface is unoccluded.
        // In wireframe there are no fills, so faces never occlude.
        ...
        if let Some((_, t, vi, gz)) = vertex
            && t <= occluder
        {
            return Some(hit(PickKind::Vertex(vi), ray_point(origin, dir, t), gz));
        }
        if let Some((_, t, li, gz)) = edge
            && t <= occluder
        {
            return Some(hit(PickKind::Linedef(li), ray_point(origin, dir, t), gz));
        }
        let thing_occluder = if face_occludes { face_t } else { f32::INFINITY };
        if let Some((tt, ti, gz)) = thing
            && tt <= thing_occluder
        {
            return Some(hit(PickKind::Thing(ti), ray_point(origin, dir, tt), gz));
        }
        let (ft, fi) = face?;
        ...
        if matches!(mode, SelectMode::Sector | SelectMode::All) {
            let s = if v.surface == SURFACE_FLOOR || v.surface == SURFACE_CEIL {
                ...
                map.lines
                    .get(v.source as usize)
                    .and_then(|l| l.front.sector)
            };
            if let Some(s) = s {
                let gz = if v.surface == SURFACE_FLOOR || v.surface == SURFACE_CEIL {
                    v.pos[2]
                } else {
                    map.sectors[s as usize].floor_height as f32
                };
    ...
    /// Ray-`t` for the thing's camera-facing billboard quad.
    /// BVH box is camera-invariant; precise quad built at resolve time.
    fn thing_quad_hit(
        &self,
        centre: [f32; 2],
        z: f32,
