...
impl PickKind {
    ...
    pub(crate) fn as_item(self) -> Option<SelItem> {
        match self {
            Self::Vertex(i) => Some(SelItem::Vertex(i)),
            Self::Linedef(i) => Some(SelItem::Line(i)),
            Self::Thing(i) => Some(SelItem::Thing(i)),
            Self::Sector(_) => None,
        }
    }
}
...
/// Möller–Trumbore, front-facing only (matches GPU CCW back-cull).
/// `det > 0` ↔ camera-facing. Returns `t > 0` or `None`.
...
/// Squared closest distance between segment `a`–`b` and the ray, and `t ≥ 0`.
/// Clamped segment–ray closest points (Ericson, Real-Time Collision Detection §5.1.9).
...
mod tests {
    use super::*;
    use crate::render::frame3d::{SURFACE_CEIL, SURFACE_FLOOR, build_mesh};
    ...
    fn ray_down_from_above_hits_floor_through_culled_ceiling() {
        ...
        let mesh = build_mesh(&map, &tris, &HashMap::new());
        ...
        let floor_h = map.sectors[s as usize].floor_height as f32;
