//! AABB BVH over mesh triangles + thing billboard boxes for O(log n) pick gather.
//! Leaf bounds are camera-invariant; rebuilt on geometry change.
...
impl MeshBvh {
    pub fn build(mesh: &[Vert3D], things: &[ThingLeaf]) -> Self {
        ...
        while i + 3 <= mesh.len() {
            ...
            b.expand(mesh[i].pos);
            b.expand(mesh[i + 1].pos);
            b.expand(mesh[i + 2].pos);
            items.push((
                b,
                Leaf::Tri {
                    tri: i,
        ...
        for t in things {
            // ±max(hw,hh) so the box contains the quad for any camera orientation.
            let [hw, hh] = t.half;
            let r = hw.max(hh);
            let c = [t.centre[0], t.centre[1], t.z + hh];
            let mut b = Aabb::empty();
...
mod tests {
    ...
    use crate::render::frame3d::build_mesh;
    ...
    fn e1m1_mesh() -> Vec<Vert3D> {
        ...
        build_mesh(&map, &tris, &HashMap::new())
    }
    ...
    /// Over a grid of downward rays, gather must not drop any triangle `pick_mesh` hits.
    #[test]
    fn gather_never_prunes_the_true_hit() {
        let mesh = e1m1_mesh();
        let bvh = MeshBvh::build(&mesh, &[]);
        // E1M1 x∈[-768,3808], y∈[-4864,-2048]; coarse grid sample.
        let mut leaves = Vec::new();
        let mut checked = 0;
        let dir = [0.0, 0.0, -1.0];
        for gx in 0..12 {
            for gy in 0..12 {
                ...
                assert!(
                    leaves
                        .iter()
                        .any(|l| matches!(l, Leaf::Tri { tri } if *tri == hit.tri)),
                    "gather dropped the pierced triangle at ({x},{y})"
                );
                checked += 1;
            }
        }
