...
/// Wall bands for one side, top to bottom; `None` if no textured band present.
fn side_bands(
    map: &EditorMap,
    line: &LineDef,
    side: &SideDef,
    ...
    let s = map.sectors.get(side.sector? as usize)?;
    ...
    let mut raw: Vec<WallBand> = Vec::with_capacity(3);
    match other
        .and_then(|o| o.sector)
...
fn rebuild(ui: &EditorWindow, state: &mut SharedState, pixel_ratio: f32) {
    let canvas = ui.global::<CanvasController>();
    let Some(index) = state.map_render.hovered_line else {
    ...
    let Some(line) = map.lines.get(index as usize) else {
        hide_wall_preview(&canvas);
        return;
    };
...
fn rebuild_sector(ui: &EditorWindow, state: &mut SharedState, pixel_ratio: f32) {
    ...
    let Some(sector) = state
        .map_render
        .hovered_sector
        .and_then(|i| state.app.map.as_ref()?.sectors.get(i as usize))
        .copied()
    else {
        hide_wall_preview(&canvas);
        return;
