//! LAUNCH tool: play-test the open map from a clicked spot; moves the launch thing on a map copy, exports a temp PWAD, spawns the engine detached.
...
pub fn plan_launch(
    ...
    if !map.things.values().any(|t| t.kind == launch_type) {
...
pub fn export_launch_wad(map: &mut EditorMap, plan: &LaunchPlan) -> Result<PathBuf, LaunchError> {
    let key = map
        .things
        .iter()
        .find(|(_, t)| t.kind == plan.launch_kind)
        .map(|(k, _)| k)
        .ok_or(LaunchError::NoLaunchThing {
            kind: plan.launch_kind,
        })?;
    map.things[key].x = plan.snapped_click[0];
    map.things[key].y = plan.snapped_click[1];

    let bytes = export_map_pwad(map, &plan.map_name, &plan.nodes).map_err(LaunchError::Export)?;
    let wad_path = env::temp_dir().join(LAUNCH_WAD);
    fs::write(&wad_path, bytes).map_err(LaunchError::Io)?;
    Ok(wad_path)
}
