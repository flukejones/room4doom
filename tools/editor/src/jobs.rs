//! Off-thread worker jobs: WAD export, BSP build (animation-only), play-test launch.
//! Pattern: `start_*` (UI) → `run_*` (worker) → `finish_*` (UI).
//!
//! BSP must not run on the Slint event loop. Worker posts [`JobOutcome`] over a
//! channel then pings `ExportController.job-done` via `upgrade_in_event_loop`.
//! Channel bridges the `Send` closure ↔ `Rc`-owning UI thread.
...
use crate::{SharedState, bsp_anim, prefs};
