...
/// Emit the editor's trimmed thing-info table (doomednum + radius per kind); pick heights come from sprite-derived extents, and the full MapObjInfo stays in the gameplay-generated file.
pub fn write_editor_things_file(data: &Data, path: PathBuf) {
    let mut out = String::from(
        "//! Editor thing info (doomednum + world radius per kind) — GENERATED WITH MULTIGEN, DO NOT EDIT BY HAND.\n\n\
    ...
    for key in &data.mobj_order {
        ...
        out.push_str(&format!(
            "    ThingInfo {{ doomednum: {doomednum}, radius: {radius} }},\n"
        ));
    }
    out.push_str("];\n");
    OpenOptions::new()
...
pub fn write_info_file(data: Data, path: PathBuf) {
    ...
    // ACTION ID ENUM
    {
        let mut actor_actions: Vec<&str> = Vec::new();
        let mut player_actions: Vec<&str> = Vec::new();
        for id in &data.action_ids {
            let variant = id.trim_start_matches("ActionId::");
            if variant.starts_with('A') {
                actor_actions.push(variant);
            } else if variant.starts_with('P') {
                player_actions.push(variant);
            }
        }
        file.write_all(
        ...
        .unwrap();
        file.write_all(b"    // Actor actions\n").unwrap();
        for v in &actor_actions {
            file.write_all(format!("    {v},\n").as_bytes()).unwrap();
        }
        file.write_all(b"    // Player actions\n").unwrap();
        for v in &player_actions {
            file.write_all(format!("    {v},\n").as_bytes()).unwrap();
        }
        file.write_all(b"}\n").unwrap();
    }
...
pub struct Data {
    ...
    /// Unique action identifiers collected during parsing (e.g.,
    /// "ActionId::AChase")
        ...
        if line.starts_with("S_") {
            ...
            if split.len() > 1 {
                ...
                if let Some(map) = states.get_mut(&split[0]) {
                    ...
                    let action_str = if split[4].to_lowercase().contains("null") {
                        "ActionId::None".to_string()
                    } else {
                        validate_field(&split[4])
                    };
                    if action_str != "ActionId::None" && !action_ids.contains(&action_str) {
                        action_ids.push(action_str.clone());
                    }
                    map.insert("action".to_string(), action_str);
...
pub fn validate_field(input: &str) -> String {
    if input.contains("*FRACUNIT") {
        // Strip FRACUNIT, store as "N.0" for f32 fields (radius, height)
        // Speed field handles FRACUNIT conversion separately in info_to_string
        ...
        // Action function -> ActionId variant
        let name = input.trim_start_matches("A_").to_lowercase();
        let variant = capitalize(&name);
        if PLAYER_FUNCS.contains(&format!("a_{name}").as_str()) {
            format!("ActionId::P{variant}")
        } else {
            format!("ActionId::A{variant}")
        }
    } else {
        input.to_string()
    }
