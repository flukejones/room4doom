...
/// Convert a speed value to raw OG Doom i32.
///
/// After `validate_field`, FRACUNIT entries become `"N.0"` (e.g. `20*FRACUNIT`
/// → `"20.0"`). Plain integers remain as-is (e.g. `"8"`). This converts to raw
/// Doom speed:
/// - `"20.0"` → `"1310720"` (20 * 65536)
/// - `"8"` → `"8"`
fn speed_to_raw(s: &str) -> String {
    if let Some(n) = s.strip_suffix(".0") {
        let v: i64 = n
            .parse()
            .expect("speed FRACUNIT multiplier must be integer");
        (v * 65536).to_string()
    } else {
        s.to_string()
    }
...
pub fn info_to_string(name: &str, info: &InfoType) -> String {
    ...
    MapObjInfo {{
        ...
        // OG default spawnhealth is 1000 (items, decorations, etc.)
        spawnhealth = info.get("spawnhealth").unwrap_or(&"1000".to_string()),
        seestate = info
            .get("seestate")
            .map(|n| if n == "0" { "StateNum::None" } else { n })
            .unwrap_or("StateNum::None"),
...
mod tests {
    ...
    fn test_states() {
        ...
        assert_eq!(state.get("action").unwrap(), "ActionId::AChase");
