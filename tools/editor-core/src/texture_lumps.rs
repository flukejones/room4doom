//! `TEXTURE<n>` + PNAMES lump encoding for custom composite textures, one lump per set (TEXTURE1, TEXTURE2, … in order); a PWAD's TEXTURE lump REPLACES the IWAD's, so these lumps must hold the FULL set (imported + edits), exactly what a project's texture sets hold after `Project::create`. Wire format: `maptexture_t` is name(8)/masked(i32,0)/width(i16)/height(i16)/columndirectory(i32,obsolete 0)/patchcount(i16), then per patch originx(i16)/originy(i16)/patch(i16 PNAMES index)/stepdir(i16)/colormap(i16); the lump itself is i32 count + i32 offsets + records, and PNAMES is an i32 count followed by 8-byte names.
...
pub enum TextureLumpError {
    ...
    /// PNAMES patch indices are i16 on the wire; more distinct patches cannot encode.
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::TooManyPatches {
                ...
                write!(
                    f,
                    "PNAMES overflow: {count} patches exceed the i16 index space"
                )
            }
        }
    }
...
impl Error for TextureLumpError {}

/// Encode one `TEXTURE<n>` lump per set (TEXTURE1, TEXTURE2, …) plus the shared PNAMES lump; `extra_patch_names` are appended to PNAMES even when no texture references them (so imported patches register), deduped against the texture-referenced names.
pub fn encode_texture_lumps(
    ...
    let mut patch_index = |name: [u8; 8]| -> Result<i16, TextureLumpError> {
        if let Some(at) = pnames.iter().position(|n| *n == name) {
            // Fits: every entry was index-guarded on push.
            return Ok(at as i16);
        }
        let index = i16::try_from(pnames.len()).map_err(|_| TextureLumpError::TooManyPatches {
            count: pnames.len() + 1,
        })?;
        pnames.push(name);
        Ok(index)
...
mod tests {
    ...
    #[test]
    fn pnames_overflow_past_i16_is_rejected() {
        let names: Vec<Name8> = (0..=i16::MAX as u32 + 1)
            .map(|i| Name8::new(&format!("P{i:07}")).expect("valid"))
            .collect();
        let err = encode_texture_lumps(&[], &names).expect_err("index space exhausted");
        assert!(matches!(
            err,
            TextureLumpError::TooManyPatches { count } if count == i16::MAX as usize + 2
        ));
    }
