//! [`EditorMap`] → vanilla WAD lumps, BSP nodes via rbsp, PWAD assembly. Record layouts (little-endian): THINGS = x/y/angle/kind/options (5×i16, 10 bytes); LINEDEFS = v1/v2/flags/special/tag/front/back (7×16-bit, 14 bytes; back sidedef 0xFFFF = none); SIDEDEFS = x_off/y_off(i16), upper/lower/middle (8-byte names), sector(i16), 30 bytes; SECTORS = floor/ceil height(i16), floor/ceil flat (8-byte names), light/special/tag(i16), 26 bytes; empty texture names encode as `"-"`. Sidedefs are embedded per line in the model and emitted in line order, front then back, so SIDEDEFS is always de-shared. Vertices unreferenced by any line are pruned on export and LINEDEFS indices renumbered to match (rbsp compacts unreferenced vertices out of VERTEXES; doombsp likewise wrote only referenced points). Sector splitting: the .dwd format merges byte-identical sectordefs while editing, and doombsp re-split them into connected components over BSP subsectors at save time (`ProcessSectors`/`RecursiveGroupSubsector`); [`export_map_pwad`] reproduces that split after `build_bsp` — subsectors sharing a vertex and an identical sector record group into one output sector, sidedef sector indices rewritten from the segs referencing them. Disable via [`ExportOptions::split_disconnected_sectors`] for maps whose sector identity is authoritative (imported from a WAD).
...
use crate::model::{DenseMap, DenseSideDef, EditorMap, Sector};
...
const DEFAULT_SKY_FLAT: &str = "F_SKY1";
/// rbsp's default BSP split cost weight.
const DEFAULT_SPLIT_WEIGHT: f64 = 10.0;
/// "No back sidedef" sentinel in a LINEDEFS record.
const NO_BACK_SIDEDEF: u16 = u16::MAX;
/// Cap on original (referenced) vertices before export, below the u16 limit so the BSP's appended split vertices still index as u16 downstream.
...
pub struct ExportOptions {
    ...
    pub sky_flat: String,
    /// Re-split value-merged sectors into connected components (doombsp save-time parity). Disable for WAD-imported maps to preserve their sector identity exactly.
...
pub enum ExportError {
    ...
    /// A void-facing side (no sector); exporting it as sector 0 would corrupt the map.
    UnenclosedSide {
        line: usize,
    },
    Wad(WadWriteError),
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UnenclosedSide {
                ...
                write!(
                    f,
                    "line {line} has a side with no sector; close the gap before export"
                )
            }
...
fn prune_vertices(map: &DenseMap) -> Result<PrunedVertices, ExportError> {
    ...
    // Original referenced vertices must index as u16 (0..=65535); the BSP then appends split vertices that also need u16 indices in SEGS/SSECTORS, so we cap originals below the limit to leave headroom rather than at the exact maximum.
...
fn flatten_sides(map: &DenseMap) -> Result<FlatSides<'_>, ExportError> {
...
fn encode_things(map: &DenseMap) -> Result<Vec<u8>, ExportError> {
...
/// A side's sector index; `check_sector_refs` ran before any caller, so `None` cannot occur.
fn sector_of(side: &DenseSideDef) -> u32 {
    side.sector.expect("check_sector_refs rejected None sides")
}

/// Convert the editor model into rbsp's input. Sidedef sector indices are the model's (pre-split) indices; rbsp only reads sector records for 3D geometry heights, which the split never changes.
fn to_bsp_input_flat(
    ...
    for (i, side) in flat.sides.iter().enumerate() {
        sidedefs.push(WadSideDef {
            ...
            sector: to_i16("sidedef sector", i, sector_of(side) as i32)?,
        });
    }
...
fn identity_assignment(map: &DenseMap, flat: &FlatSides<'_>) -> SectorAssignment {
    let mut sectors = Vec::with_capacity(map.sectors.len());
    sectors.extend_from_slice(&map.sectors);
    let side_sector = flat.sides.iter().copied().map(sector_of).collect();
    SectorAssignment {
        sectors,
        side_sector,
    }
...
/// doombsp `ProcessSectors` parity: group subsectors into connected components (shared vertex + identical sector record), one output sector per component, sidedef sectors reassigned from the segs touching them. Union-find root with path compression.
...
fn split_assignment(map: &DenseMap, flat: &FlatSides<'_>, output: &BspOutput) -> SectorAssignment {
    let num_ss = output.subsectors.len();

    // Value-identity of each input sector (the merge doombsp's UniqueSector performed; already true for dwd-imported maps, harmless otherwise).
    ...
    // Union-find over subsectors: same vertex + same def --> same sector. Vertex buckets iterate in sorted order so the partition (and thus the exported byte image) is deterministic across runs.
    ...
    // Sides no seg touched (degenerate geometry): first component of their def, or a fresh sector if the def produced no subsectors at all.
    for (i, side) in flat.sides.iter().enumerate() {
        if side_sector[i] == u32::MAX {
            let s = sector_of(side) as usize;
...
/// Full export pipeline: range checks, BSP build, sector split, lump encoding, PWAD assembly. Returns the complete PWAD byte image.
...
/// [`export_map_pwad`] plus extra lumps appended after the map (project TEXTURE1/2 + PNAMES when custom textures exist).
...
/// [`export_map_pwad_with_lumps`] that also returns the BSP build trace (in build order) for the editor's animation overlay. The PWAD bytes are identical to the untraced call.
...
/// Reject sector refs that cannot export: index past the sector list, or no sector at all.
fn check_sector_refs(map: &DenseMap) -> Result<(), ExportError> {
    let n = map.sectors.len();
    for (line, l) in map.lines.iter().enumerate() {
        for side in [Some(&l.front), l.back.as_ref()].into_iter().flatten() {
            match side.sector {
                Some(s) if s as usize >= n => {
                ...
                Some(_) => {}
                None => {
                    return Err(ExportError::UnenclosedSide {
                        line,
                    });
                }
            }
...
mod tests {
    ...
    use super::*;
    use crate::model::{DenseLineDef, Thing, Vertex};
    use crate::name8::Name8;
    use crate::wad_import::import_wad_map;
    use crate::{LineFlags, ThingFlags};
    use wad::WadData;

    /// Keyed map for the public export API; fixtures stay dense.
    fn keyed(dense: DenseMap) -> EditorMap {
        EditorMap::from_dense(dense).expect("fixture refs valid")
    }
    ...
    fn side(sector: u32, middle: &str) -> DenseSideDef {
        DenseSideDef {
            x_offset: 0,
            y_offset: 0,
            top_tex: Name8::EMPTY,
    ...
    /// Clockwise square: the front (right) side of every line faces the interior, the Doom convention for a closed room.
    ...
    fn one_square_map() -> DenseMap {
        let (verts, edges) = square_at(0.0, 0.0);
        let lines = edges
            .iter()
            .map(|&(a, b)| DenseLineDef {
                v1: a as u32,
                v2: b as u32,
    ...
    /// Two disconnected squares whose sides reference ONE merged sector record — the dwd-import shape that doombsp re-splits at save time.
    fn two_disconnected_squares_map() -> DenseMap {
        let (mut verts, edges) = square_at(0.0, 0.0);
        let (verts2, _) = square_at(512.0, 0.0);
        let base2 = verts.len() as u32;
        verts.extend(verts2);

        let mut lines: Vec<DenseLineDef> = edges
            .iter()
            .map(|&(a, b)| DenseLineDef {
                v1: a as u32,
                v2: b as u32,
            ...
            .collect();
        lines.extend(edges.iter().map(|&(a, b)| DenseLineDef {
            v1: base2 + a as u32,
            v2: base2 + b as u32,
            flags: LineFlags::BLOCKING,
            special: 0,
    ...
    fn out_of_range_vertex_rejected() {
        ...
        let err = export_map_pwad(&keyed(map), "E1M1", &ExportOptions::default())
    ...
    #[test]
    fn side_without_sector_rejected() {
        let mut map = one_square_map();
        map.lines[0].front.sector = None;
        let err = export_map_pwad(&keyed(map), "E1M1", &ExportOptions::default())
            .expect_err("void-facing side cannot export");
        assert!(
            matches!(
                err,
                ExportError::UnenclosedSide {
                    line: 0
                }
            ),
            "{err}"
    ...
    /// Round-trip through a real engine-shaped map: shareware E1M1 in, PWAD out, imported again. Split disabled — the WAD's sector identity is authoritative.
    #[test]
    fn e1m1_export_reimports_identically() {
        ...
        let path = env::temp_dir().join("editor_core_e1m1_roundtrip.wad");
        fs::write(&path, &bytes).expect("temp pwad writes");
        let rewad = WadData::new(&path);
        let remap = import_wad_map(&rewad, "E1M1").expect("exported pwad imports");
        fs::remove_file(&path).ok();

        let (map, remap) = (map.to_dense(), remap.to_dense());
        assert_eq!(remap.things, map.things);
        assert_eq!(remap.sectors, map.sectors);
        assert_eq!(remap.lines.len(), map.lines.len());
        // Vertex indices renumber (unreferenced originals are pruned on export), so endpoints compare by coordinate.
