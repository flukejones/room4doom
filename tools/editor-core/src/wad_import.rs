//! Import a vanilla map (THINGS/LINEDEFS/SIDEDEFS/VERTEXES/SECTORS) from a
//! WAD into the editor model.
//!
//! - WAD sectors are already first-class records; they convert 1:1 with no
//!   merging (the merge only exists for .dwd ingestion).
//! - All VERTEXES entries are kept, including node-builder seg vertices, so
//!   linedef indices are preserved; unreferenced vertices are harmless.
//! - WAD sidedefs may be shared between linedefs (rare, and illegal in
//!   vanilla); sides are embedded per line here, so shared sidedefs are
//!   de-shared by value construction.
//! - Thing coordinates are NOT grid-snapped on WAD import; snapping is a
//!   .dwd-load behavior only. WAD data is authoritative as-is.
//!
//! The reverse direction — `EditorMap` into `rbsp::BspInput` and WAD lumps —
//! is the export path and lives in `wad_export` (phase 2).
...
use crate::model::{EditorMap, GROWTH_HEADROOM, LineDef, Sector, SideDef, Thing, Vertex};
...
impl std::error::Error for WadImportError {}
...
pub fn import_wad_map(wad: &WadData, map_name: &str) -> Result<EditorMap, WadImportError> {
    ...
    // Vanilla VERTEXES holds the linedef-endpoint vertices followed by the
    // BSP build's seg-split vertices; the editor only needs the former.
...
/// Collect a map lump's records with exact pre-sizing. `RecordIter` has no
/// size hint, so the record count comes from a first counting pass (iteration
/// is allocation-free).
...
mod tests {
    ...
    /// The .dwd fixture is John Romero's released DoomEd source for E1M1;
    /// doombsp compiled it into the shipped WAD. Comparing resolved data (not
    /// indices — the dwd dedups endpoint vertices and merges sectors by
    /// value, while doombsp re-split merged sectors into connected components
    /// at save time) proves both importers reconstruct the same map. The
    /// WAD's 85 sectors deduplicate by value to exactly the dwd merge count.
    #[test]
    fn wad_import_matches_dwd_fixture_record_for_record() {
        let from_wad = e1m1();
        let from_dwd = parse_dwd(FIXTURE).expect("fixture parses");
        ...
        for (i, (dt, wt)) in from_dwd.things.iter().zip(&from_wad.things).enumerate() {
            assert_eq!(
                ...
                (
                    wt.x & crate::dwd::THING_SNAP_MASK,
                    wt.y & crate::dwd::THING_SNAP_MASK,
                    wt.angle,
                    wt.kind,
                    wt.options
                ),
