//! Profiling harness for `sector_loops` (the sector-outline tracer, a large-map-load hot path): re-traces every sector N times for a sampler; `cargo run --release -p editor-core --example profile_sector_loops -- <iwad> <map> [pwad] [iters]` (e.g. `-- ~/DOOM/doom2.wad MAP19 ~/DOOM/sunder.wad 200`).
...
fn main() {
    ...
    let third = args.next();
    let pwad = third.as_deref().map(PathBuf::from).filter(|p| p.exists());
    ...
    assert_eq!(
        per / iters,
        all / iters,
        "batch and per-sector loop counts differ"
    );
}
