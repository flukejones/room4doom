...
use map_data::pvs::{
    FLOOR_RATIO_HIGH, FLOOR_RATIO_LOW, LIGHT_DIFF_THRESHOLD, OPENING_MEDIUM, OPENING_NARROW, WEIGHT_FLOOR_RATIO_HIGH, WEIGHT_FLOOR_RATIO_LOW, WEIGHT_LIGHT_DIFF, WEIGHT_MEDIUM, WEIGHT_MOVER, WEIGHT_NARROW, WEIGHT_SEALED, WEIGHT_SPECIAL
};
use map_data::{MapData, PVS, RegionPortal};
use std::sync::Arc;
...
pub struct ViewerData {
    ...
    /// Subsector-to-region mapping (empty if no PVS built).
    ss_to_region: Vec<usize>,
    /// Portal segments between regions (empty if no PVS built or loaded from
    /// cache).
...
pub fn extract_viewer_data(
    ...
    // Carve 2D subsector polygons via BSP traversal (full pipeline with drift
    // correction and edge vertex insertion, matching the 3D renderer).
    ...
    let ss_to_region = pvs.map_or(Vec::new(), |p| p.subsector_to_region().to_vec());
    let region_portals = pvs.map_or(Vec::new(), |p| p.region_portals().to_vec());
...
impl MapViewerApp {
    ...
    fn save_pvs(&mut self) {
        let Some(pvs) = &self.data.pvs else {
            self.state.status_msg = Some("No PVS data to save".into());
            return;
        };
        match PVS::get_pvs_cache_path(&self.data.map_name, self.data.map_hash) {
            Ok(path) => match pvs.save_to_file(&path) {
                Ok(()) => {
                    let msg = format!("PVS saved to {}", path.display());
                    log::info!("{msg}");
            ...
            Err(e) => {
                self.state.status_msg = Some(format!("Cache path error: {e}"));
            }
        }
    }
    ...
    fn load_pvs_cache(&mut self) {
        ...
        match PVS::load_from_cache(&self.data.map_name, self.data.map_hash, ss_count) {
            Some(pvs) => {
                // Cache doesn't have region/portal data, so those stay empty
                self.data.ss_to_region = pvs.subsector_to_region().to_vec();
                self.data.region_portals = pvs.region_portals().to_vec();
                self.data.pvs = Some(pvs);
    ...
    fn rebuild_pvs(&mut self) {
        ...
        let vis_count = (0..ss_count)
            .map(|s| pvs.get_visible_subsectors(s).len())
            .sum::<usize>();
        self.data.ss_to_region = pvs.subsector_to_region().to_vec();
        self.data.region_portals = pvs.region_portals().to_vec();
        self.data.pvs = Some(pvs);
    ...
    fn handle_input(&mut self, response: &egui::Response) {
        ...
        if is_primary_drag && ctrl && self.state.drag_tool == DragTool::None {
            if let Some(pos) = response.interact_pointer_pos() {
                ...
                if shift {
                    self.state.drag_tool = DragTool::RectSelect { start: map_pos };
                } else {
                    self.state.drag_tool = DragTool::LineProbe { start: map_pos };
                }
            }
        }
    ...
    fn draw_layers(&mut self, painter: &egui::Painter, vc: Pos2) {
        ...
        if self.state.show_all_portals && !self.data.region_portals.is_empty() {
            ...
            for portal in &self.data.region_portals {
                for &(v1, v2) in &portal.segs {
                    let p1 = self.map_to_screen(v1, vc);
                    let p2 = self.map_to_screen(v2, vc);
        ...
        // PVS highlight portals: portals between any two visible regions
        if self.state.show_pvs_portals && !self.data.region_portals.is_empty() {
            if let Some(sel) = self.state.selected_subsector {
                if let Some(&sel_region) = self.data.ss_to_region.get(sel) {
                    if let Some(pvs) = &self.data.pvs {
                        let vis_regions = pvs.region_visibility();
                        let visible: std::collections::HashSet<usize> =
                            if sel_region < vis_regions.len() {
                                vis_regions[sel_region].iter().copied().collect()
                            } else {
                                std::collections::HashSet::new()
                            };
                        let w = (self.state.zoom * 0.5).clamp(1.0, 6.0);
                        let glow =
                            Stroke::new(w + 4.0, Color32::from_rgba_unmultiplied(255, 120, 0, 60));
                        let core =
                            Stroke::new(w, Color32::from_rgba_unmultiplied(255, 160, 0, 240));
                        for portal in &self.data.region_portals {
                            if !visible.contains(&portal.region_a)
                                || !visible.contains(&portal.region_b)
                            {
                                continue;
                            }
                            for &(v1, v2) in &portal.segs {
    ...
    fn draw_sidebar(&mut self, ui: &mut egui::Ui) {
        ...
        ui.checkbox(&mut self.state.show_all_portals, "All region portals");
        ...
        ui.heading("PVS");
        let has_pvs = self.data.pvs.is_some();
        ui.horizontal(|ui| {
    ...
    fn draw_hover_overlay(&self, painter: &egui::Painter, viewport_rect: egui::Rect) {
        ...
        if let Some(lid) = self.state.hovered_linedef {
            if let Some(ld) = self.data.linedefs.get(lid) {
                ...
                if let Some(back_id) = ld.back_sector_id {
                    let w = Self::compute_linedef_weight(
                        &self.data.sectors,
                        ld.front_sector_id,
                        back_id,
                        ld.special,
                    );
                    spans.push((lbl, " wt:".into()));
                    spans.push((val, format!("{:<5.0}", w)));
                }
                lines.push(spans);
            }
        }
    ...
    /// Compute edge weight for a two-sided linedef from viewer sector data.
    /// Mirrors the logic in pvs.rs compute_edge_weight using the same
    /// constants.
    fn compute_linedef_weight(
        sectors: &[ViewSector],
        front_id: usize,
        back_id: usize,
        special: i16,
    ) -> f32 {
        let (Some(sa), Some(sb)) = (sectors.get(front_id), sectors.get(back_id)) else {
            return 0.0;
        };
        let mut weight = 0.0f32;

        if special != 0 || sa.is_mover || sb.is_mover {
            weight += WEIGHT_SPECIAL;
            if sa.is_mover || sb.is_mover {
                weight += WEIGHT_MOVER;
            }
        }

        let opening = (sa.ceiling_height.min(sb.ceiling_height)
            - sa.floor_height.max(sb.floor_height))
        .max(0.0);
        if opening <= 0.0 {
        ...
        let ha = sa.ceiling_height - sa.floor_height;
        let hb = sb.ceiling_height - sb.floor_height;
        if ha > 0.0 && hb > 0.0 {
            let ratio_diff = (sa.floor_height / ha - sb.floor_height / hb).abs();
        ...
        let light_diff = (sa.light_level as i32 - sb.light_level as i32).unsigned_abs();
    ...
    fn build_ss_spans(data: &ViewerData, ss_id: usize, ss_count: usize) -> Vec<(Color32, String)> {
        ...
        if let Some(ss) = data.subsectors.get(ss_id) {
            ...
            if let Some(&region) = data.ss_to_region.get(ss_id) {
                spans.push((lbl, " region:".into()));
                spans.push((val, format!("{:<5}", region)));
            }
            ...
            if let Some(pvs) = &data.pvs {
                ...
                let md = pvs.max_flood_depth(ss_id);
                if md > 0 {
                    spans.push((lbl, " depth:".into()));
                    spans.push((val, format!("{md}")));
                }
            }
    ...
    fn draw_subsector_info_static(
        ...
        if let Some(ss) = data.subsectors.get(ss_id) {
            ...
            if let Some(pvs) = &data.pvs {
                ...
                let md = pvs.max_flood_depth(ss_id);
                if md > 0 {
                    ui.monospace(format!("{:<W$}{md}", "Max depth:"));
                }
            }
            if let Some(s) = data.sectors.get(sid) {
                if s.is_mover {
