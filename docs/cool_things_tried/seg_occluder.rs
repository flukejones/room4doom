...
use level::{BSP3D, OcclusionSeg, Sector, is_subsector, subsector_index};
use math::Angle;
use glam::{Vec2, Vec3};
...
impl SegOccluder {
    pub fn new(fov: f32, screen_width: f32, screen_height: f32) -> Self {
        ...
        let fov_half_tan = Angle::<f32>::new(fov_half).tan();
    ...
    pub fn process_seg(
        ...
        match occ_seg.back_sector_id {
            ...
            Some(back_id) => {
                ...
                if back.ceilingheight.to_f32() <= front.floorheight.to_f32()
                    || back.floorheight.to_f32() >= front.ceilingheight.to_f32()
                {
                    is_solid = true;
                } else {
                    is_solid = false;
        ...
        if is_solid {
            ...
            let front = &sectors[occ_seg.front_sector_id];
            let opening_ceil = front
                .ceilingheight
                .to_f32()
                .min(back.ceilingheight.to_f32());
            let opening_floor = front.floorheight.to_f32().max(back.floorheight.to_f32());
