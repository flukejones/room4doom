...
use level::LevelData;
use level::{
    BSP3D, PvsData, Sector, SurfaceKind, SurfacePolygon, WallType, is_subsector, subsector_index,
};
...
use crate::rasterizer::interpolation::TriangleInterpolator;
use crate::scene::render::{TextureSampler, sample_sky_pixel};
use crate::{SkyRend, Software3D};

const TILE_SIZE: usize = 8;
use crate::rasterizer::LIGHT_SCALE;
...
impl Software3D {
    ...
    fn prepare_and_emit_polygon(
        ...
        for i in 0..self.rasterizer.screen_vertices_len {
            let j = (i + 1) % self.rasterizer.screen_vertices_len;
            let vi = self.rasterizer.screen_vertices_buffer[i];
            let vj = self.rasterizer.screen_vertices_buffer[j];
        ...
        let screen_verts =
            &self.rasterizer.screen_vertices_buffer[..self.rasterizer.screen_vertices_len];
        let tex_coords = &self.rasterizer.tex_coords_buffer[..self.rasterizer.tex_coords_len];
    ...
    pub fn draw_view_bench_edge_spans(
        ...
        if cfg!(feature = "hiz_prev_frame") {
            self.rasterizer.depth_buffer.soft_reset();
        } else {
        ...
        let depth_ptr = self.rasterizer.depth_buffer.depths_raw_ptr();
        let depth_stride = self.rasterizer.depth_buffer.width();
        let tile_min_ptr = self.rasterizer.depth_buffer.tile_min_ptr();
        let tile_covered_ptr = self.rasterizer.depth_buffer.tile_covered_ptr();
        let tiles_x = self.rasterizer.depth_buffer.tiles_x();
