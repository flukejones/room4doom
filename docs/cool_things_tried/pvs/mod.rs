...
use game_config::PreprocessPvsMode;
use glam::Vec2;
use log::{info, warn};
use wad::WadData;

/// Build and cache PVS for a single map.
///
/// `flat_lookup` resolves flat names to indices (pass `|_| None` if not needed).
/// Returns `Ok(true)` if PVS was built, `Ok(false)` if cache already existed.
pub fn preprocess_level(
    wad: &WadData,
    map_name: &str,
    mode: PreprocessPvsMode,
    flat_lookup: impl Fn(&str) -> Option<usize>,
) -> Result<bool, Box<dyn std::error::Error>> {
    let hash = wad.map_bsp_hash(map_name).unwrap_or_default();
    let cache_path = traits::RenderPvs::cache_path(map_name, hash)?;
    ...
    let mut level_data = crate::level_data::LevelData::default();
    level_data.load(map_name, flat_lookup, wad, None, None);

    let crate::level_data::LevelData {
        subsectors,
        segments,
        bsp_3d,
        sectors,
    ...
    match mode {
        PreprocessPvsMode::Cluster => {
            let cluster = pvs_cluster::PvsCluster::build(
                subsectors, segments, bsp_3d, sectors, linedefs, nodes, *start_node,
            );
            cluster.save_to_cache(map_name, hash)?;
        }
        PreprocessPvsMode::Full | PreprocessPvsMode::Mightsee => {
            let pvs = pvs2d::PVS2D::build(
                subsectors,
                segments,
                bsp_3d,
                nodes,
            ...
            pvs.save_to_cache(map_name, hash)?;
        }
    }
    Ok(true)
}

/// Get all map names present in a WAD (E1M1..E9M9 and MAP01..MAP99).
pub fn get_all_map_names(wad: &WadData) -> Vec<String> {
    let mut maps = Vec::new();
    for episode in 1..=9 {
        for map in 1..=9 {
            let name = format!("E{}M{}", episode, map);
            if wad.lump_exists(&name) {
                maps.push(name);
            }
        }
    }
    for map in 1..=99 {
        let name = format!("MAP{:02}", map);
        if wad.lump_exists(&name) {
            maps.push(name);
        }
    }
