...
### Runtime — render_bsp with Hi-Z node culling (no sky polys, no solidsegs, spawn point)

Unified BSP traversal: immediate front-to-back render with `is_full()` early-out and Hi-Z AABB rejection at internal BSP nodes. Replaces the old collect-sort-render pipeline. Nodes hiz-culled = entire BSP subtrees skipped because their projected AABB was fully behind existing depth.

| Metric | MAP03 no PVS | E6M6 no PVS | E6M6 PVS | MAP20 no PVS |
|---|---|---|---|---|
| Submitted | 593 | 1,095 | 1,081 | 2,455 |
| Frustum-clipped | 0 | 0 | 0 | 0 |
| Culled | 35 | 22 | 22 | 254 |
| Early-depth | 0 | 2 | 2 | 0 |
| No-draw | 235 | 446 | 432 | 1,884 |
| Rendered | 323 | 625 | 625 | 317 |
| Subsectors | 237/237 | 818/818 | 791/818 | — |
| Nodes hiz-culled | 41 | 230 | 230 | — |
| FPS | ~1,317 | ~961 | ~975 | ~300+ |

Compared to the previous n-poly collect-sort-render path (same spawn points):

| Metric | MAP03 old | MAP03 new | E6M6 old | E6M6 new | MAP20 old | MAP20 new |
|---|---|---|---|---|---|---|
| Submitted | 6,534 | 593 | 2,432 | 1,095 | 27,666 | 2,455 |
| Early-depth | 5,704 | 0 | 582 | 2 | 17,924 | 0 |
| Rendered | 307 | 323 | 578 | 625 | 218 | 317 |
| FPS | ~377 | ~1,317 | ~1,050 | ~961 | ~96 | ~300+ |

MAP03 sees the largest improvement: 6,534→593 submitted (91% reduction), ~377→~1,317 fps (3.5x). All maps show near-zero early-depth rejections — Hi-Z node culling eliminates polygons before they reach per-polygon depth testing. Rendered counts increase slightly due to BSP front-to-back order vs exact depth sort. MAP20 sees a 3x fps improvement from Hi-Z node culling at spawn.
...
| **n-poly + Hi-Z node cull, no PVS** | **—** | **1,095** | **625** | **~961** |
| **n-poly + Hi-Z node cull, PVS** | **—** | **1,081** | **625** | **~975** |

> Note: All entries above "n-poly" used triangle-fan tessellation and included sky polygons (floor/ceiling surfaces in sky sectors). The n-poly rows use walls as quads and flats as n-gons, without PVS. The first n-poly row skips sky polygons; the "+ sky" row includes them. Hi-Z node cull rows use unified `render_bsp` with immediate front-to-back rendering and Hi-Z AABB rejection at BSP internal nodes (230 nodes culled at spawn).
...
### E6M6 (Sigil 2) — voxel_parallax_doomII.pk3, full PVS, spawn point

| Metric | No Voxels | With Voxels |
|---|---|---|
| Voxel objects | — | 100 |
| Behind camera | — | 23 |
| Hi-Z culled (object) | — | 52 |
| Normal-culled (slices) | — | 950 |
| Slices rendered | — | 1,222 |
| Polys submitted | 2,409 | 2,409 |
| Polys rendered | 626 | 626 |
| FPS | ~720 | ~417 |

Voxel pipeline: 3-level culling (object bbox → per-quad frustum+hi-Z → per-texel hi-Z), double-sided slice quads (3 axis pairs), clip-space incremental projection, scanline parallelogram fill with precomputed edge slopes, front-to-back depth with hi-Z tile updates.

### CRT Stretch Removal Impact (commit 9aa3997)

Removed `projection_matrix.y_axis *= 240.0/200.0` CRT stretch that emulated Doom's 320x200→4:3 display. Without the 1.2x vertical squash, the effective vertical FOV is wider — more geometry visible per frame, more pixel fill.

| Metric | Before (with stretch) | After (no stretch) |
|---|---|---|
| E6M6 FPS softbuffer (spawn point) | ~999+ | ~830 |
| E6M6 FPS pixels (spawn point) | — | ~720 |

~17% drop (softbuffer) from increased vertical visibility alone. Same view position, same pipeline. Pixels backend has additional overhead vs softbuffer.
...
- Hi-Z node culling (rejecting entire BSP subtrees via projected AABB depth test) largely replaces PVS for occluded scenes — E6M6 no-PVS with Hi-Z node cull (1,095 submitted, ~961 fps) outperforms old full-PVS (1,694 submitted, ~1,034 fps) without any precomputation. PVS remains valuable for large open maps where geometry doesn't occlude well (MAP20 worst case ~85 fps is fill-rate bound, not culling-bound)
