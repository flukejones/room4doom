...
Key features:
- Software rendering:
  - Classic Doom style (2.5D)
  - Full 3D with N-poly flats, wall quads
- Cheelo Voxels support (3d rendering only)
- Classic OPL2 emulation (love the old blips and bloops of Sound Blaster)
- 3 different graphics backends:
  - Pixels (pure rust)
...
The code will dramatically change 0ver time.

# 24/03/26

Totally reversed course on FixedPoint. Ripped out all the floats in gameplay and software25d, replaced with a FixedT type whcih can be 32bit or 64bit.

# 24/03/26

Full 3D software rendering done. Uses N-poly and quads. Initally tried a full triangle mesh and while it worked, it tanked performance. N-poly basically doubled or tripled it.

```bash
CLI options for the game-exe

Options:
  -v, --verbose     verbose level: off, error, warn, info, debug
  -i, --iwad        path to game-exe WAD
  -p, --pwad        path to patch WAD
  -w, --width       resolution width in pixels
  -h, --height      resolution height in pixels
  -f, --window-mode window mode: windowed, borderless, exclusive
  --vsync           enable vsync (true/false)
  --refresh-rate    refresh rate in Hz for exclusive fullscreen (e.g. 60, 144)
  -H, --hi-res      set hi-res mode for software rendering (true/false)
  -n, --no-monsters disable monsters
  --dev-parm        developer mode. Screen is cleared with green colour for
                    seg/flat drawing leak checks
  ...
  --demo            play this demo lump name immediately and exit when done
                    (e.g. demo1, demo2)
  --preprocess-pvs  preprocess PVS data for loaded WADs and exit; mode: full,
                    mightsee, cluster
  --frame-interpolation
                    enable frame interpolation for smooth rendering between tics
                    (true/false)
  --dbg-draw-overlay
                    debug overlay mode (mutually exclusive): sector_id, depth,
                    overdraw, wireframe
  --dbg-draw-mod    debug draw modifiers (comma-separated): outline, normals,
                    clear_<hex>, alpha_<0-255>, no_depth
  --voxels          path to voxel directory or PK3 archive containing KVX files
  --help, help      display usage information
```

Added voxel support. Super hacked up, it works by slicing the model in to quads and sparse textures, no raycasting, no triangulated quad mesh. It draws by projecting the quads from teh slice that are in view and drawing the block colour using a sparse texture of sorts (skip blank runs because there are a lot).
