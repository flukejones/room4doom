    ...
    pub fn parse(wad_data: &WadData, map_name: &str) -> Option<Self> {
        ...
        if let NodeLumpType::Extended(t) = node_type {
            if t.is_uncompressed() {
                ...
                return Some(Self::parse_compressed(lump, t));
            }
        }
        None
    }

    fn parse_compressed(lump: &Lump, etype: ExtendedNodeType) -> Self {
        let compressed = &lump.data[4..];
        let mut decoder = ZlibDecoder::new(compressed);
        let mut decompressed = Vec::new();
        decoder
            .read_to_end(&mut decompressed)
            .expect("Failed to decompress ZNOD/ZGLN/ZGL2 data");
        info!(
            "Decompressed {} bytes from {} compressed bytes",
            decompressed.len(),
            compressed.len()
        );
        // Prepend 4-byte dummy header so parse_uncompressed can start at offset 4
        let mut data = vec![0u8; 4];
        data.extend_from_slice(&decompressed);
        let decompressed_lump = Lump {
            name: lump.name.clone(),
            data,
        };
        Self::parse_uncompressed(&decompressed_lump, etype)
    }
