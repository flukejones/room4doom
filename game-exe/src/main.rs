...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if options.preprocess_pvs {
        ...
        if !options.iwad.is_empty() {
            let wad_path: PathBuf = options.iwad.into();
            preprocess_pvs_for_wad(&wad_path)?;
        }
...
fn preprocess_pvs_for_wad(wad_path: &Path) -> Result<(), Box<dyn Error>> {
...
fn process_map_pvs(wad: &WadData, wad_name: &str, map_name: &str) -> Result<(), Box<dyn Error>> {
    ...
    let cache_path = gameplay::PVS::get_pvs_cache_path(wad_name, map_name, hash)?;
