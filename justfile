...
lint-editor-patterns:
	...
	# `pub use` lines and doc-comment intra-doc links (`///`, `//!`), where a
	# multi-segment path is the idiomatic link form, not a code-site qualifier.
	check 'crate::[a-z_]+::[A-Za-z]' '*.rs' ':[0-9]+:[[:space:]]*(pub )?use |:[0-9]+:[[:space:]]*//[/!]' \
